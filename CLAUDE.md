...
- **Directory structure:**
  ...
  │   ├── core/        AgenstormSettings, AgenstormConfigurable, AgenstormBundle, AgenstormAppScope, AgenstormSettingsListener, AgenstormNotifications
...
- **Internal API:** none. The Marketplace review rejected 1.1.0 over fifteen `@ApiStatus.Internal` usages and they were all replaced or removed; `verifyPlugin` now reports zero, and that is the bar every change has to keep. `SPEC.md` §2 lists what is still allowed and why it is not internal (`FrameTitleBuilder` service override, `commentsReferenceProvider`, registry key `ide.mac.os.wintabs.version2`, subclassing git4idea's `GitBranchWidget`). Anything marked internal → stop and ask; an `@ApiStatus.Experimental` hook is fine and the plugin carries plenty.
