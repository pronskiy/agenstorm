...
- **Directory structure (target):**
  ...
  ```
  What exists today: `core/` (`AgenstormBundle`, `AgenstormSettings` light `@Service` → `agenstorm.xml`, `AgenstormConfigurable` Tools → Agenstorm), `links/` (`FileLocation` + `FileLocationMatch`, `FileLocationParser`, `FileLocationResolver`, `FileLocationSymbol` + `FileLocationNavigationTarget`, `FileLocationSymbolReference`) and `links/markdown/` (`MarkdownLocationReferenceProvider`, `LocationLinkInspectionSuppressor`, registered in `META-INF/agenstorm-markdown.xml`; `agenstorm-php.xml` and `agenstorm-git.xml` are still empty). Tests mirror the packages under `src/test/kotlin/com/pronskiy/agenstorm/` (settings, smoke, parser, resolver, symbol reference, Markdown provider + fixture test) with fixtures in `src/test/testData/links/{resolver,md}/`. Workflows: `.github/workflows/{build,release,run-ui-tests}.yml` (`build.yml` = buildPlugin, check, verifyPlugin, release draft; no Qodana/Codecov).
