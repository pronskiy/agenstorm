...
| 2026-09-09 | Epic E reversed on its central mechanism: the macOS window tabs stay on (they are what merges the projects into one window) and only their row is hidden; new step E1.6, decision 32, new "One window" guardrail | Roman Pronskiy (decision), Claude (investigation, text) |
| 2026-09-09 | Epic F gains Phase F4: markers are painted as text, not with the scheme's folded-code styling; decision 33 | Roman Pronskiy (decision), Claude (investigation, text) |
| 2026-09-09 | Epic H Phase H3 started: block quotes render as a card with an accent bar (H3.1, H3.2, part of H3.4); thematic breaks (H3.3) still open. Decision 34 | Roman Pronskiy (decision), Claude (text) |
...
| 2026-09-21 | **1.8.0 cut and released** — Epic Q, tables rendered in place and one cell edited in place. `check` green, ZIP built, verifier Compatible on both IDEs with zero internal API; Marketplace review passed in about twenty minutes; draft published as Latest | Roman (decision, sign-offs), Claude (runs, publish, text) |
| 2026-09-22 | **1.8.1 cut and released** — the terminal maximize toggle puts the caret in the terminal (J1.7). `check` green, ZIP built, verifier Compatible on both IDEs with zero internal API and the 1.8.0 counts; Marketplace review passed in about thirteen minutes; draft published as Latest, `release.yml` green with "1.8.1 is being served" | Roman (decision, sign-off, the go-aheads), Claude (fix, runs, publish, text) |
...
**Now on:** **Epic I — terminal output enhancers**, started 2026-09-22 on Roman's "let's go with Epic I" (the re-deciding the pointer asked for: Roman read the summary and said go). Order: I1.1 → I1.2 → I1.3 (the spike: a sandbox run and a §6 decision row, Roman's call on what he sees) → the I1 guardrails → I2 → I3. **I1.1 and I1.2 are ✅** (2026-09-22, 15 + 13 tests; two of the three I1 guardrails closed by them). **I1.3 is ✅** (2026-09-22, decision 70: fold regions survive everything the robot harness could drive — appended output, CR rewrites, resize, the alternate buffer, 175 k lines and the 1 MiB trim; a placeholder click expands). Phase I1 is closed but for Roman's two eye checks (⌘K, typing). **Decision 71 (2026-09-22): Reworked 2025 only** — I2.3 cut, the classic engine just has to stay inert. **I2.1 is ✅** (2026-09-22, 7 tests; no gutter icon — the terminal hides that area). **I2.2, I2.4 and Phase I3 are ✅** (2026-09-22: the viewer, then the rules folder with hot reload, the settings table, the README section; 11 + 12 tests). **Epic I is built** — every step ✅ or ❌ by decision 71. **Next: the Epic I guardrail run** (`runIde` on the demo project: a real `var_dump` folding and opening as a tree, 10 000 lines with 200 matches, a hostile regex file, a broken file, the Classic engine inert, the off switch, the log), then the 1.9.0 cut. **Before that:** **1.8.1 is released** (2026-09-22): pushed as `666f2dd`, `build.yml` green and the draft made, the **Marketplace upload** workflow (run 35722773377) dispatched on Roman's word, review passed — the update server listed 1.8.1 at 13:53, about thirteen minutes after the upload — and the draft published as Latest by Claude at 13:53 on Roman's "publish the draft once review passes" (`https://github.com/pronskiy/agenstorm/releases/tag/1.8.1`, `agenstorm-1.8.1-signed.zip`). `release.yml` (run 35723992092) came back green with *"1.8.1 is being served - this release is installable"* and opened no changelog PR, because `patchChangelog` had already run in the cut. **`pluginVersion` stays 1.8.1** until there is something to release. The one open item from this release is a by-eye check of the invokeLater swap (J1.7's notes) on the next sandbox run. **Next: Epic I** (terminal output enhancers), as before. **Before that:** **1.8.1 is cut and pushed** (2026-09-22, on Roman's "cut 1.8.1 and push"): `pluginVersion = 1.8.1`, the changelog section patched (`[1.8.1] - 2026-09-22`), `agenstorm-1.8.1.zip` built (909 KB, sha256 `37296250…`), `./gradlew check` green and the verifier **Compatible on PS-262.10968.76 and IU-262.10968.63 with zero internal usages** (76 experimental, 11 deprecated — the 1.8.0 counts; the first verifier run said 14, because `IdeFocusManager.doWhenFocusSettlesDown` is deprecated in 2026.2 in all three overloads, so the re-request now runs from `Application.invokeLater` under the toggle's modality — a mechanism change made *after* Roman's sandbox sign-off, so the next sandbox run should press ⌘⌥M once more). It carries the J1.7 fix. **Uploaded 2026-09-22 on Roman's "dispatch the marketplace upload":** `build.yml` green on all four jobs with the 1.8.1 draft made, the **Marketplace upload** workflow (run 35722773377) green — Sign and upload, and `agenstorm-1.8.1-signed.zip` (933 KB) attached to the draft. **What is left:** the Marketplace review — the update server listing 1.8.1 is the signal — and then the draft published (`https://github.com/pronskiy/agenstorm/releases/tag/1.8.1`). **Before that:** **J1.7 landed 2026-09-22** — Roman's ⌘⌥M left the caret in the editor; the terminal is focused again once the pane has reshaped, signed off in the sandbox ("looks good to me"), and it sits in `[Unreleased]` as a fix, so the next number is **1.8.1** at Roman's call. **1.8.0 is released** (2026-09-21, the whole epic in one day): pushed as `6d24219`, `build.yml` green and the draft made, the **Marketplace upload** workflow dispatched from the terminal, review passed — the update server listed 1.8.0 at 20:57 — and the draft published as Latest by Claude on Roman's "you publish" at 21:16 with `agenstorm-1.8.0-signed.zip` (`https://github.com/pronskiy/agenstorm/releases/tag/1.8.0`). `release.yml` came back green and opened no changelog PR, because `patchChangelog` had already run in the cut. **`pluginVersion` stays 1.8.0** until there is something to release: 1.8.1 for a fix, 1.9.0 for a feature. **The M and N guardrail lines were exercised 2026-09-21** in one sandbox pass, all good, log clean; only Epic M's *Reachable* row waits for what Find Action showed. **Next: Epic I** (terminal output enhancers, 10 steps), the biggest thing left and worth re-deciding before starting.

**Before that:** **1.8.0 is cut** (2026-09-21): `pluginVersion = 1.8.0`, the changelog section patched (`[1.8.0] - 2026-09-21`), `agenstorm-1.8.0.zip` built (907 KB, sha256 `7a67eefd…`), `./gradlew check` green and the verifier **Compatible on PS-262.10968.76 and IU-262.10968.63 with zero internal usages** (76 experimental and 11 deprecated — both the 1.7.0 counts, the deprecated one after `LinkDestinations` moved off `ReadAction.compute`). It carries **Epic Q**: tables rendered in place and one cell edited in place. **Pushed and dispatched 2026-09-21 on Roman's "push and dispatch":** `main` at `6d24219`, `build.yml` green on all four jobs with the 1.8.0 draft made, the **Marketplace upload** workflow (run 35639757400) green — Publish Plugin and the signed ZIP (`agenstorm-1.8.0-signed.zip`, 931 KB) attached to the draft. **What is left:** the Marketplace review — the update server listing 1.8.0 is the signal — and then Roman publishes the draft (`https://github.com/pronskiy/agenstorm/releases/tag/1.8.0`).

**Earlier:** **Epic Q — Markdown tables rendered in place**, added 2026-09-21 at Roman's request ("I want to be able to view markdown tables really nicely" — the raw table "looks like complete mess"). Spec'd in one brainstorm; the four calls are decisions 66–68. **Phase Q1 is closed** (2026-09-21: `TableModel` + `TableModelBuilder` with thirteen tests, `TableLayout` with nine, both guardrails ✅). **Phase Q2 is signed off** (2026-09-21: Q2.1–Q2.5 built with 34 tests, the sandbox run "all good", the one shutdown leak it surfaced fixed in `78b1574`). **Phase Q3 is rewritten** around Roman's ask to edit one cell rather than reveal the table (decision 69). **Phase Q3 is built** (Q3.1–Q3.4, 2026-09-21, 13 more tests). **Next: the Phase Q3 guardrail run** — `runIde` on the demo project, the six rows of the Q3 table; then the 1.8.0 cut. Q1 is tests-only; the first `runIde` sign-off is the Phase Q2 guardrail table.

**Earlier:** **1.7.0 is released** (2026-09-19, the whole run in one evening): pushed as `9e1fd26`, `build.yml` green and the draft made, the **Marketplace upload** workflow dispatched from the terminal (the `.env` token may trigger `workflow_dispatch`, verified), review passed in about fourteen minutes — the update server listed 1.7.0 at 21:18 — and the draft published as Latest with `agenstorm-1.7.0-signed.zip` (`https://github.com/pronskiy/agenstorm/releases/tag/1.7.0`). `release.yml` came back green and opened no changelog PR, because `patchChangelog` had already run in the cut. **`pluginVersion` stays 1.7.0** until there is something to release: 1.7.1 for a fix, 1.8.0 for a feature.
...
**Two things carried knowingly into 1.5.0:** five guardrail lines across M and N were never exercised (M's *Markdown only*, *No restart*, *Reachable*; N's *Both ways*, *Not greedy*), all covered by unit tests; and M hides the layout actions from Find Action too, so while it is on there is no way to open the preview — documented rather than fixed.
...
2. **`build.yml`'s Create Release Draft step will fail on the next push while `pluginVersion` is still 1.4.0**, because `gh release create 1.4.0 --draft` collides with the published release; *Remove Old Release Drafts* only clears drafts. So bump the version before pushing more work, or expect that one job red.

**Release CI, reworked 2026-09-11 so it tells the truth about review.** `publishPlugin` succeeding means *uploaded and accepted*, not *installable*: a new version goes through review, which for this plugin is real (1.1.0 was rejected). Observed directly on 1.4.0 — while it was in review the update server `/plugins/list/?pluginId=…` served 1.3.0 only, and an hour later, once approved, it served 1.4.0. **The API's per-update `approve` flag is not the signal**: it read `true` for 1.4.0 the whole time it was unavailable. The update server list is what IDEs read, so that is what the CI compares against.
- `build.yml` drafts the release as a **pre-release**, and skips creating a draft at all when a release for that version already exists (the collision that used to red the next push).
- `release.yml` triggers on **`published` only**. Both other types are wrong here: `prereleased` does not fire for a pre-release published from a draft — which is exactly this repo's flow, so it would silently never run — and `released` fires again when the watcher promotes, which would re-publish a version the Marketplace already has. It also guards `publishPlugin` behind a served-version check, and its changelog PR step now exits cleanly when there is nothing to commit instead of reding a run whose publish already succeeded.
- **`marketplace-watch.yml`** (new) runs daily and on demand: any published pre-release whose version the update server is serving gets promoted with `gh release edit --prerelease=false --latest`. The shell was exercised against the live API before it shipped, matching 1.4.0 and 1.3.0 and correctly declining a version that is not served.

**Superseded:** the 1.4.0 cut, 2026-09-11: `pluginVersion = 1.4.0`, the changelog section patched, `agenstorm-1.4.0.zip` built, `./gradlew check` green and the verifier **Compatible on PS-262.10315.130 and IU-262.10315.125 with zero internal usages** (50 experimental, 4 deprecated — all three counts unchanged since 1.2.0). It carries **Epic L** (notification popups that hide themselves) on top of everything 1.3.0 held, which **was never published** — so 1.4.0 is what ships J, K and L together. **1.3.0 was published on 2026-09-11**: Roman uploaded the signed ZIP to the Marketplace by hand, and the GitHub draft release `1.3.0` was published on tag `1.3.0` (commit `75e5524`, `main`) with the changelog section as its notes — **deliberately without triggering `release.yml`**, whose `publishPlugin` step is unconditional and would have re-uploaded a version the Marketplace already had. The Release workflow was disabled for the publish and re-enabled straight after; `gh run list` confirms no Release run fired, and the note about a stale draft `1.1.0` (id 385481892) elsewhere on this page is obsolete — that draft is gone. `agenstorm-1.3.0-signed.zip` is attached by hand, since the pipeline that normally attaches it was the thing being skipped. **Watch the distributions directory when doing this**: a later `verifyPlugin` run rebuilds `agenstorm-<current version>.zip` from *current* source, so `build/distributions/agenstorm-1.3.0.zip` was a 1.3.0-named archive that already contained Epic L. The `-signed` copy, dated to the cut and carrying no Epic L classes, is the one that matches the release. **What is left is Roman's**: the unpushed commits, the 1.4.0 GitHub release (`release.yml` fires on `released`/`prereleased` and signs and pushes the ZIP to the Marketplace), and the guardrails still open — Epic J's and Epic K's, plus Epic L's four deferred ones.

**Superseded:** **Publishing 1.3.0.** The version was cut (2026-09-10): `pluginVersion = 1.3.0`, the changelog section is patched, `agenstorm-1.3.0.zip` is built, and the verifier is **Compatible on PS-262.10315.130 and IU-262.10315.125 with zero internal usages** (50 experimental, 4 deprecated — both unchanged since 1.2.0). It carries **Epic J** (terminal ⇄ editor maximize, ⌘⌥M won from Extract Method by a promoter rather than by editing a keymap — decisions 45 and 46) and **Epic K** (the IDE as the terminal's `$EDITOR`: Claude Code's Ctrl+G, `git commit`, anything that reaches for an editor — decisions 43 and 44). **What is left is Roman's**: 24 commits to push, the GitHub release to publish — `release.yml` fires on `released`/`prereleased` and signs and pushes the ZIP to the Marketplace — and then the guardrails he has not run yet (two projects open, a profile that exports `EDITOR`, `git commit`, the non-project-file bar, Ctrl+C mid-edit, the off switch, a daily-driver log run), plus the correction to IJPL-221866.

**2026-09-10 — Epic L, notification popups that hide themselves.** Roman asked whether balloons can be hidden automatically; §7 keeps the survey, and he chose the timed auto-dismiss over the IDE's all-or-nothing switches. **Phase L1 is built**: balloons fade after 5 s instead of the platform's 10 s / 5 min, errors keep their long timer unless opted in, and nothing persisted by the platform is touched — the one call re-arms the balloon's own alarm, so the pause-while-the-IDE-is-in-the-background behaviour comes along for free. `./gradlew check` green, and `verifyPlugin` Compatible on both IDEs with **zero internal usages** and 50 experimental / 4 deprecated, all three unchanged from 1.3.0. **Roman's first run found the commit popup still on screen**; root cause was ours and is fixed (L1.7, the smart-vs-plain delay above), with the regression pinned by a test against a real `BalloonImpl`. **Confirmed working 2026-09-11** — "it works now, popup disappears after 5 seconds" — with a clean log, which closes the *It hides* and *Log* guardrails. It sits in `[Unreleased]`, not in 1.3.0. **Two guardrails are signed off** (*It hides*, *Log*); the other four are 🔄 and **deferred to a daily-driver run at Roman's call, 2026-09-11** — the same way Epic J's log guardrail was left. Three of them (*Nothing is lost*, *Errors stay*, *Off switch*) are pinned by test and want only a confirming look; *Background pause* is the one where a test can pin the branch but not the real activation behaviour. The sandbox keeps `com.pronskiy.agenstorm.notifications` at DEBUG, so whoever runs it next gets the evidence for free. Epic L is otherwise **done**: 20 tests, `check` green, verifier unchanged.
...
**2026-09-09 — Phase H3, block quotes.** Roman asked for them next, rendered "like the code block". **H3.1** and **H3.2** are built (decision 34): every `>` folds to a single space so no column moves, one card and one accent bar per outermost quote, behind the new `liveMarkupBlockQuotes` setting. `./gradlew check` green from clean. **H3.3**, thematic breaks, is still open. Roman's first look found two things, both fixed the same day: a caret revealed one `>` at a time (the markers now share the quote's span, so the group reveals as one) and the bar stayed while the raw markers showed (it now paints only while they are hidden). A second look found GitHub alerts left raw; they are `ALERT` elements, now handled alongside `BLOCK_QUOTE`. **H3.3** followed the same day: thematic breaks fold whole and get a rule drawn across the row, no card. **Phase H3 closed 2026-09-09** — Roman signed off both remaining guardrails ("all good"); the bar's position needed no inset after all. No ERROR or SEVERE and no plugin frames in that sandbox session. Epic H now stands at H1 ✅, H2 🔲 (after 1.0), H3 ✅.
...
| Internal API usage | **None.** The Marketplace review rejected 1.1.0 over fifteen `@ApiStatus.Internal` usages; `verifyPlugin` reports zero since 2026-09-10 and that is now a release gate, not a tolerance — and since 2026-09-14 the build enforces it: `INTERNAL_API_USAGES` is in `failureLevel`, so a reintroduced internal call fails `verifyPlugin` instead of appearing in a report nobody reads (step O1.0). What replaced them: the frame-title refresh goes through `UISettings.fireUISettingsChanged()` (the platform's own path into `EditorsSplitters.updateFrameTitle`) instead of `IdeFrameEx.setFileTitle`; the status-bar branch widget extends git4idea's public `GitBranchWidget` and takes the branches popup and the branch icon from its `protected` members instead of calling `GitBranchesTreePopupOnBackend.create`; the project tabs take `main.toolbar.Project` through `ActionManager.replaceAction` instead of subclassing `ProjectToolbarWidgetAction`, keeping the instance they displace so the stock widget comes back when the feature is switched off or the plugin unloads; and Epic B, whose hook (`scratchLanguageFilter`) has no public equivalent, was cut and then rebuilt on public API instead — its own `NewScratchFile` action, swapped into the platform's slot the same way (Phase B2, decision 39). What is left in impl packages is not internal: the `FrameTitleBuilder` service override, `commentsReferenceProvider`, and the registry key `ide.mac.os.wintabs.version2`. `@ApiStatus.Experimental` is a separate category and stays fine — 46 of them, and the review did not flag it | Public Marketplace plugin: JetBrains reject internal API usage outright, and a missing hook must log and no-op, never throw |
...
### Epic B — New Scratch File allowlist  ·  MVP, rebuilt on public API 2026-09-10

**Cut and brought back inside a day.** Phase B1 built the feature on `scratchLanguageFilter`, an
`@ApiStatus.Internal` extension point; the Marketplace review rejected the plugin over internal API, so B1 was
removed wholesale (`b7db79d`, decision 35). Roman then asked for it back on public API only, which **Phase B2**
does: Agenstorm's own `NewScratchFile` action, in the platform's own action slot. Two things improve in the move —
the filter is on *languages*, which retires the dialect limitation decision 14 had to document, and the popup
gains an "All Languages…" way out. Decision 39. Phase B1 below is kept as the record of what the internal hook
did and why it had to go.

**Goal:** The New Scratch File popup shows only the languages the user allows, and the platform's own popup is
back the moment the feature is switched off.
**Success metrics:** Popup shows exactly the allow-listed languages plus the way out; no internal API; a list
...
#### Phase B2 — The popup, on public API only

| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| B2.1 | `ScratchLanguageAllowList`: which languages the popup offers, in the order the list names them | ✅ | `filter(candidates, allowed)`. An entry matches a language by id or display name; only if it matches none does it fall back to the language's **file type** name, which is what a 1.0 list holds. The fallback has to be second: `JavaScript` is both a language and the file type of ECMAScript 6 and ActionScript, so matching both at once would drag the dialects back in — the first cut of this class did exactly that and the test caught it. 9 cases in `ScratchLanguageAllowListTest`, against real registered languages (a `Language` registers itself by class, so a test cannot make its own) |
| B2.2 | `NewScratchFileAction`: the popup, and the scratch it creates | ✅ | Candidates from `LanguageUtil.getFileLanguages()`, the same set the platform's popup starts from; `JBPopupFactory.createPopupChooserBuilder` with the file-type icon and speed search; last row "All Languages…" reopens the popup unfiltered and sorted by `LanguageUtil.LANGUAGE_COMPARATOR`. Creation: `ScratchFileCreationHelper.EXTENSION.forLanguage` for the language's template and caret offset, an editor selection as the text, then `ScratchRootType.createScratchFile`, which takes the write action and picks the next free `scratch_N` name through `create_new_always`. Every one of those is public and unannotated in build 262 — **`ScratchFileActions.doCreateNewScratch`, which would have done the whole job, is `@ApiStatus.Internal` and was checked before any of this was written** |
| B2.3 | `ScratchActionInstaller` + `core/ActionSlot`: hold the platform's `NewScratchFile` slot while the feature is on | ✅ | Second user of the swap Phase E1 needed, so the lifecycle moved into `core/ActionSlot` (take, give back, identity check, `CustomActionsListener.fireSchemaChanged()`) and both features call it. Keeping the platform's action **id** is what keeps the keymap shortcut, the File → New entry and the editor popup item working; `replaceAction` keeps the position in every parent group but does **not** copy the presentation, so the action carries its own text |
| B2.4 | Settings: the group returns, editing languages | ✅ | `scratchAllowedLanguages` (default `Plain text, Markdown, PHP, JavaScript, HTML`) with an "Add Language…" chooser over `LanguageUtil.getFileLanguages()`. `scratchAllowedFileTypes` stays as a legacy field that `loadState` folds in once and clears, so a list customized under 1.0 or 1.1 is not lost; `AgenstormSettingsTest` covers the fold-in, the XML shape and the edit/apply/reset round trip |

**Known deviations from the platform's popup**, all deliberate: no recent-first (LRU) ordering — with a handful
of allowed languages there is nothing to rank; selections are read from the editor only, since the extractors for
trees, tables and the terminal are internal; and the `<override-text place="EditorPopup"/>` the platform's action
declares is not reproduced, so the console editor popup reads "Scratch File" rather than "Create Scratch File".
...
**Goal:** On macOS, open projects appear as tabs inside the main toolbar row (left slot, where the project widget is), so the window has one header row instead of two; tabs show project names only; the projects share **one window** — the platform's macOS window tabs stay on and only their own row is hidden, so a tab click selects a tab rather than raising a second window.
...
- Native tabs are `WindowTabsComponent` (a `JBTabsImpl`) driven by `MacWinTabsHandlerV2` (`platform/platform-impl/src/com/intellij/ui/mac/`). Two separable things hang off registry `ide.mac.os.wintabs.version2` (default `true`, restart required): the **window merging** — `MacWinTabsHandlerV2.initFrame` → `JdkEx.setTabbingMode(frame, windowId, null)` plus `NSWindow setAllowsAutomaticWindowTabbing:YES`, giving one `NSWindowTabGroup` driven with `addWindow:` / `insertWindow:atIndex:` / `removeWindow:` / `setSelectedWindow:` — and the **row** the tabs are drawn in. Re-parenting the row is not viable (see §2); hiding it is.
- `JdkEx.getTabbingModeInvocator()` is `(!ExperimentalUI.isNewUI() || Registry.is("ide.mac.os.wintabs.version2", true)) && SystemInfo.isJetBrainsJvm && SystemInfo.isMacOSBigSur && Registry.is("ide.mac.bigsur.window.with.tabs.enabled", true)`. Under the New UI, turning the first key off makes `isTabbingModeAvailable()` false, so `IdeRootPane` installs no row **and** no tab group is ever formed. It logs the five booleans: `=== TabbingMode: disabled (…) ===`.
- The row is a `JPanel` on the frame's root pane under client property `WINDOW_TABS_CONTAINER_KEY`, added to the layered pane. `IdeRootPane.CustomHeaderRootLayout` stacks `customFrameTitle` (on macOS New UI the `MacToolbarFrameHeader`: traffic lights **and** the main toolbar) → `menuBar` → that row → `toolbar` → content, each contributing height **only when `isVisible()`**. So the row is *below* the toolbar, and hiding it collapses it with nothing else moving. `MacWinTabsHandlerV2` itself hides it whenever the group is down to one window (`createTabBarsForFrame` / `removeFromFrame` are the only two places that touch its visibility).
- `Window.transparentTitleBarHeight` (28) is set alongside the row but has no layout effect — its only reader is `UIUtil.getTransparentTitleBarHeight` → `MacMainFrameDecorator`, the double-click-to-zoom handler.
- `MacWinTabsHandler.switchFrameIfPossible(JFrame, boolean)` is public and does `selectNextTab:` / `selectPreviousTab:` on the group.
...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| E1.1 | `NativeTabsRegistryGuard` (`ProjectActivity`): keep `ide.mac.os.wintabs.version2` **on** — it is what merges the projects into one window — and turn it back on once for installs 1.0 disabled it in, notifying "Restart to apply" | ✅ | **Reversed 2026-09-09** (decision 32). 1.0 set the key to `false` so the tab row would go away; the key also gates `JdkEx.isTabbingModeAvailable()`, so that gave every project its own window — the flaw Roman reported. The guard now only undoes itself: `nativeTabsDisabledByAgenstorm` → set the key back to `true`, clear the flag, offer the restart. A user's own `false` is left alone (their strip switches windows, as on Windows/Linux). Proof of the old chain, from the sandbox log: `=== TabbingMode: disabled (true,true,true,true false) ===` — the only false was our key. `NativeTabsRegistryGuardTest` rewritten (7 cases: never-off, 1.0 restore once, restore with the feature off, user's own choice, flag-only, non-mac, missing key) |
...
| E1.5 | Verifier check of the override; if extending the internal `ProjectToolbarWidgetAction` is rejected, fall back to a standalone `CustomComponentAction` with a minimal own "project dropdown" for the feature-off state (record decision) | ✅ | `verifyPlugin` 2026-09-05: Compatible against PS-262.10315.130 and IU-262.10315.125; internal usages 4 → 13, all from subclassing `ProjectToolbarWidgetAction` (class reference, constructor, `update`/`createCustomComponent`/`updateCustomComponent` overrides) plus the four sanctioned earlier. `RecentProjectListActionProvider.getActions(Project)` turned out to be `@Internal` too and was replaced by the public boolean overload. No fallback needed (decision 18). Re-run after Phase E2 (2026-09-05): still Compatible on both IDEs, still 13 internal usages  **Settled 2026-09-10:** the fallback this step reserved is what the plugin now does, minus the hand-built dropdown — the stock widget itself is restored into the slot, so feature-off is stock behaviour without naming the stock class. Decision 36 |
| E1.6 | `NativeTabStrip`: hide the platform's tab row per frame so the header stays one line, and give back only a row we hid | ✅ | New with decision 32. The row is the `JPanel` under root-pane client property `WINDOW_TABS_CONTAINER_KEY`; `CustomHeaderRootLayout` gives it height only while visible, so `isVisible = false` collapses it — the same layout the platform produces for a one-window group. Installed from `TabsStartupActivity` on the EDT, with a `ComponentListener` that re-hides it when `createTabBarsForFrame` brings it back; the disposal is registered on the project. Nothing is re-parented. Feature off restores it to the platform's own rule (visible once a second window joined) and only when a client property marks it as ours, matching `nativeTabsDisabledByAgenstorm` / `navBarHiddenByAgenstorm`. `containerOf(rootPane) != null` doubles as "this window is in a tab group", which `ProjectTabActions.switchTo` uses to skip bounds mirroring on macOS. Null row (non-macOS, tabbing unavailable) → debug log, no-op. `NativeTabStripTest` (6 cases) |

**Steps (detail):**

- **E1.1 — Registry guard.** Deliverable: `tabs/NativeTabsRegistryGuard.kt`. Reads `Registry.get("ide.mac.os.wintabs.version2")` and only ever writes `true`, and only when `State.nativeTabsDisabledByAgenstorm` says 1.0 wrote `false`; `RegistryValue.setValue` persists across restarts. Notification group `Agenstorm` with "Restart Now" → `ApplicationManager.getApplication().restart()`. `SystemInfo.isMac` guard; a missing key is logged and the flag kept. Also run from the settings page on apply.
- **E1.6 — Hiding the platform row.** Deliverable: `tabs/NativeTabStrip.kt`. `rootPane.getClientProperty("WINDOW_TABS_CONTAINER_KEY") as? JComponent` → `isVisible = false` + `rootPane.revalidate()/repaint()`; a `ComponentAdapter.componentShown` on that panel re-hides it after `createTabBarsForFrame`, disposed with the project. `apply(container, featureOn, tabCount)` is the whole rule and is what the tests drive; `isTabbed(project)` (the row exists ⇒ the platform found tabbing mode available for this frame) gates the bounds mirroring in `ProjectTabActions.switchTo`.
...
- **E1.4 — Interactions.** Deliverable: handlers in `ProjectTabsPanel`.
  - Left click: `switchTo(target)`: if `mirrorWindowBounds` (default on), neither project is in a macOS tab group (`NativeTabStrip.isTabbed` — inside one there is a single window to move) and neither frame is fullscreen (`ProjectFrameHelper.getFrameHelper(frame)?.isInFullScreen != true`), set `targetFrame.bounds = currentFrame.bounds`; then `ProjectUtil.focusProjectWindow(target, true)`.
...
| E3.2 | `BranchStatusBarWidget` + `Factory` (id `agenstorm.branch`, `agenstorm-git.xml`): current branch of the repository for the focused file (else the project's first), `AllIcons.Vcs.Branch`, tooltip = repository root; updates on `GIT_REPO_CHANGE` and editor switches; click → branches popup under the widget | ✅ | `CustomStatusBarWidget` (owns its `JBLabel`, so the same component can move into the central slot); text = branch, else 8-char revision, else "no branch"; popup via `GitBranchesTreePopupOnBackend.create(...).showUnderneathOf(label)` with `Git.Branches` as `LinkageError` fallback. `BranchStatusBarWidgetTest` (4)  **Reworked 2026-09-10 (decision 37):** the widget extends git4idea's `GitBranchWidget` and takes the branches popup from its `protected getWidgetPopup` and the icon from `getIcon(GitRepository)`, so `GitBranchesTreePopupOnBackend` is no longer named and the branch shows incoming/outgoing arrows. `CustomStatusBarWidget` still supplies the label, which is what E3.3's placement moves |
...
| Off switch | Toggling off restores a plain Markdown editor instantly (no leftover folds) | ✅ | Same pass, through the settings state and `ProjectViewActionInstaller.sync()` — what Settings → Apply calls. Off: `NewFile`, `NewDir`, `RenameElement` and `PhpNewFile` back to the stock classes and Duplicate hidden; on again: all four ours, Duplicate visible. No restart. The settings page itself was not clicked |
...
#### Phase F4 — Markers look like text, not like folded code (post-1.0)

| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| F4.1 | `FoldPlaceholderStyle`: override `FOLDED_TEXT_ATTRIBUTES` for editors live markup owns, so a • or ☐ is painted with the editor's default text attributes | ✅ | Roman, 2026-09-09: "in markdown rendered lists, the bulletpoints are underscored. I think they should not be". Three of the twelve kinds fold to a visible placeholder (`BULLET`, `CHECKBOX_OFF`, `CHECKBOX_ON`); the rest are zero width and paint nothing, which is why only bullets showed it. A collapsed placeholder is painted with the scheme's folded-text attributes, and his Catppuccin Mocha (`themes/mocha.xml`) sets `BACKGROUND 313244` + `EFFECT_TYPE 2` (`BOXED`) there. There is no per-region hook: `EditorPainter$Session.getFoldRegionAttributes` merges the selection, `FoldingModelEx.getPlaceholderAttributes()` and the defaults, and `FoldingModelImpl.updateTextAttributes()` is `editor.colorsScheme.getAttributes(FOLDED_TEXT_ATTRIBUTES)`. Public API only, no new §2 entry: `EditorEx.getColorsScheme()` always returns the editor's own `EditorColorSchemeDelegate`, whose `setAttributes` writes to a per-editor map that `getAttributes` reads first, and `reinitSettings()` → `FoldingModelImpl.refreshSettings()` re-reads it. Installed from the controller's `init`, restored in `dispose`, skipped for a scheme that already paints folded text plainly (a full editor re-init is not worth spending for nothing). `FoldPlaceholderStyleTest` (7 cases) |

**Exit guardrails — Phase F4**
...
#### Phase H2 — The card: rounded painting, language chip, copy  ·  ⏸️ skipped until there is feedback

**Roman, 2026-09-16:** skip until we get some feedback (decision 60). Nothing below is started beyond the `liveMarkupCodeBlocks` switch that shipped with H1.

| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| H2.1 | `CodeBlockHighlighterRenderer : CustomHighlighterRenderer` — rounded rect, insets, hairline border | ⏸️ | After 1.0 |
| H2.2 | Language chip and copy action as an after-line-end inlay on the header row | ⏸️ | After 1.0; H1 already leaves the header row empty for it |
| H2.3 | Settings: `liveMarkupCodeBlocks`, `liveMarkupCodeBlockCard`, `liveMarkupCodeBlockCopy` | ⏸️ | `liveMarkupCodeBlocks` shipped in 1.0 with its settings row; the other two are after 1.0, with H2.1 and H2.2 |
| H2.4 | Tests plus the soft-wrap and theme-switch check | ⏸️ | After 1.0 |
...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| H3.1 | Collector emits `QUOTE_MARKER` ranges, one per `>`, and a `BLOCK_QUOTE` block per outermost quote or GitHub alert | ✅ | 2026-09-09. Deviation from the step's wording: the range is the `>` **only**, replaced by a single space, so hiding it — and revealing it again at the caret — never moves a column, at any nesting depth. And the markers are found by scanning the quote's lines, not from the tree: a PSI dump showed the parser keeps only the first `>` as a `MarkdownTokenTypes.BLOCK_QUOTE` leaf and leaves the continuation markers wherever the line landed, inside the paragraph for a wrapped line and as plain `WHITE_SPACE` before a list or a nested quote. `dropSwallowedQuoteMarkers` keeps the output disjoint: a closing ``` inside a quote is one token starting at the same offset as that line's `>`, so the fence range already hides it. The walk continues into the quote, so its emphasis, links, lists and fences still fold. Guardrail feedback the same day: giving each marker its own span put each in its own `FoldingGroup`, so a caret revealed one `>` at a time; every marker of a quote now carries the quote's span, and the group is revealed as a whole — the same idiom that pairs the two markers of `**bold**`. Guardrail feedback: a GitHub alert (`> [!IMPORTANT]`) was left raw, because the parser gives it `MarkdownElementTypes.ALERT` rather than `BLOCK_QUOTE` — an AST dump showed the same shape otherwise, `>` markers and all, plus an `ALERT_TITLE` leaf. Both element types now sit in a `QUOTE_LIKE` token set that drives the branch and the outermost-only ancestor check, and the `[!…]` title is left exactly as written for the plugin's own styling and gutter icon |
| H3.2 | Card plus a left accent bar per quote, drawn only while the markers are hidden | ✅ | 2026-09-09. One bar per outermost quote rather than one per nesting level (decision 34): the nested markers already fold to spaces, so the depth still reads in the indent. `MarkdownBlockRenderer.background` became per-kind (`BLOCK_QUOTE`, else `CODE_FENCE`, else the same mix fallback); the bar is a `CustomHighlighterRenderer` on the same `LINES_IN_RANGE` highlighter, filling a 2 px scaled rect from the block's first line to its last in the column the `>` markers vacated, coloured from `BLOCK_QUOTE_MARKER`'s foreground or the card mixed further toward the foreground. `barBounds` is pure, so the geometry is tested without a paint. Guardrail feedback the same day: the bar stayed while the block showed its raw `>` markers, doubling the cue — it now paints only when the quote's first `>` is inside a collapsed region (`firstQuoteMarker` + `getCollapsedRegionAtOffset`; folding never changes the document, so the marker is always there to look up) |
| H3.3 | `HORIZONTAL_RULE` folded and drawn as a full-width line | ✅ | 2026-09-09. The token is one leaf holding exactly the rule characters — `---`, `***`, `___` or `- - -` — so it folds whole and the row is left empty, keeping its number, as decision 31 has it for a fence's ``` lines. A break is the one block kind with **no** card: `background` returns null for it and the highlighter exists only to hang `RuleLineRenderer` on, which fills a scaled 1 px rect centred in the row, across `scrollingModel.visibleArea` rather than the content component (which is as wide as the file's longest line). Coloured from `MarkdownHighlighterColors.HRULE`'s foreground, else the editor background mixed toward the foreground. Drawn only while the token is folded, like the quote bar. `ruleBounds` is pure, so the geometry is tested without a paint. `ThematicBreakTest` (10 cases) — including that `---` under a paragraph is a setext heading and must be left alone, which is why the first draft of the in-quote test failed |
| H3.4 | Settings `liveMarkupBlockQuotes`, `liveMarkupRules`; tests | ✅ | `liveMarkupBlockQuotes` shipped 2026-09-09 with its settings row (named for the block kind, not `liveMarkupQuotes`). Tests: `quotes.md` fixture, `BlockQuoteCollectorTest` (9 cases: the whole rendered document, one-char markers, content still folded, outermost-only cards, option off, the line scan, disjointness, block-marker reveal) and four more in `MarkdownBlockRendererTest`. `liveMarkupRules` shipped with H3.3 the same day, with its own settings row; `collector.md` gained a `---` so `testKindsCoverExactlyTheMarkers` still covers every `MarkupKind` |

**Exit guardrails — Phase H3**

| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| Quotes | A quote and a GitHub alert each show one card and one bar, markers hidden; a caret anywhere reveals the block whole and the bar steps aside | ✅ | Roman, 2026-09-09 ("works great") after two rounds of feedback |
| Breaks | A `---` line shows a rule across the viewport and no card; the caret on it brings the characters back and the rule goes | ✅ | Roman, 2026-09-09 ("all good") |
| Bar position | The accent bar sits where it should against the gutter | ✅ | Roman, 2026-09-09 ("all good"): `contentComponent.insets.left` reads right as it is, so no inset was added |
...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| I1.1 | `EnhancerRule` + `RuleParser`: JSON in, validated rules out, precise errors | ✅ | 2026-09-22. `terminal/enhance/`. Read by hand from `Json.parseToJsonElement` rather than `@Serializable` — the messages have to name the file, the field and what was expected, and `RuleParseException(source, field, expected)` carries them as fields for the I3.2 balloon. Regexes compile at parse time (the `PatternSyntaxException` description and index go into the message), `render` is case-insensitive, an unknown field is an error naming it (a typo'd optional field silently ignored is worse), and the summary's `{n}` references are checked against `start`'s group count. One addition to the format: `{line}` is the whole first line and the default summary, because `{0}` is only what the regex covered — `^Fatal error:` would have folded to just that. 15 cases in `RuleParserTest` |
| I1.2 | `BlockDetector`: text + rules → blocks, off the EDT, with a per-rule time budget | ✅ | 2026-09-22. `detect(text, rules, from)` returns `Detection(blocks, resumeFrom)`: the blocks, and where the next scan starts — the opener of a block whose `end` has not arrived yet, or the end of the text — so the controller rescans only the tail and a block that closes later is seen whole. Line by line, first enabled rule wins, blocks never overlap, no depth counting (the built-in `^…$` anchors handle nested `var_dump` on their own: inner openers are indented), `maxLines` exceeded means the opener was plain text. The budget is **per regex invocation** (one `find()` on one line): a `CharSequence` whose `charAt` checks the clock every 256 reads and throws, `StackOverflowError` treated the same; a rule that blows it is disabled for the detector's life and reported once. Found on the way: the textbook `(a+)+$` is defused by this JDK's engine (0 ms on 40 chars), so the fixture is `^(.*a){20}$`, which runs for hours untimed and is cancelled in 23 ms here. The five built-in rule files landed now under `resources/terminal/rules/` because the fixtures need them (I2.4 keeps the loading and override half). 13 cases in `BlockDetectorTest` over five output fixtures in `testData/terminal/enhance/` |
| I1.3 | **Spike** — can light fold regions live in a reworked-terminal output editor? Gates I2.1 | ✅ | 2026-09-22, **yes** — decision 70. The spike code is the first cut of I2.1 and stays: `TerminalEnhancerController` (debounced scan of the appended tail off the EDT, one light region per block, trim handling), `TerminalEnhancerService` (one controller per output editor, built-in rules), `TerminalEnhancerEditorListener` (`editorFactoryListener` in `agenstorm-terminal.xml`; the terminal marks its output editor *after* creating it, so the check runs one event later), setting `terminalEnhancerEnabled`. Attached by `TerminalDataContextUtils.isOutputModelEditor`, never to the alternate-buffer editor |
...
| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| Detector correctness | `BlockDetectorTest` green on all five built-in fixtures, including nested `var_dump` and a truncated block at end of output | ✅ | 2026-09-22: nested `var_dump` is one block from the outer opener to the outer brace; the truncated one yields no block and a `resumeFrom` at its opener, and the same text completed and rescanned from there yields it whole |
| Regex safety | The catastrophic-backtracking fixture is cancelled inside its budget; the test asserts the detector returns and the rule is marked disabled | ✅ | 2026-09-22: `^(.*a){20}$` on forty `a`s, budget 20 ms — the case takes 23 ms, the rule is in `disabledRuleIds`, reported exactly once across two calls, and the stack-trace rule still finds its block in the same text |
| Spike recorded | A decision row in §6 states plainly whether fold regions survive in the reworked terminal, with the observed behaviour per case | ✅ | Decision 70, 2026-09-22: they survive every case the harness could drive; ⌘K and a human's own typing are the two left for Roman's eye |

#### Phase I2 — Rendering

| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| I2.1 | `TerminalEnhancerController`: light fold regions collapsed to the summary, per output editor | ✅ | 2026-09-22. `syncNow()` for tests and the toggle; a `FoldingListener` that rescans from the start of a region someone else removed (our own batch stays quiet, like Epic F); `TerminalEnhancerService.applySettings` attaches or detaches every output editor of the project, driven by `TerminalEnhancerStartupActivity` on `AgenstormSettingsListener`; the *Terminal output* group in the settings page with the one toggle. **No gutter icon**, the spec's "and a gutter icon" is dropped: the terminal creates its editor with `setGutterIconsShown(false)` and `setFoldingOutlineShown(false)`, and turning either on would change how every terminal looks — the placeholder is the one click target, which decision 70 showed works. 7 cases in `TerminalEnhancerControllerTest` over a plain editor: the collapsed region and its summary, the tail-only rescan keeping earlier regions, a block waiting for its end, a trim through a block moving the scan position and dropping the cut region, a foreign removal undone on the next sync, a user's expansion respected, dispose taking every region |
| I2.2 | Structured tree viewer for `tree` and `json` renders | ✅ | 2026-09-22. `terminal/enhance/viewer/`: `PayloadNode` (key, the value's text as the output wrote it, children), `PayloadTreeParsers` (line-based, tolerant parsers for `var_dump`, `print_r` and `var_export`, JSON through `kotlinx.serialization`, and a line-by-line fallback so the viewer never refuses to open; a `tree` rule's format is **sniffed from the first line**, so user rules carry no format field), `PayloadViewer` (a `JBPopupFactory` component popup over a `Tree`: first two levels open, speed search, context menu with Copy Node / Copy Subtree / Copy Raw Output, size remembered). Opened from the placeholder: the controller's `EditorMouseListener` consumes a click on a collapsed `tree`/`json` region of ours, the way Epic F's checkbox click is consumed, so the region stays folded; a `fold` region expands as the platform would; hand cursor over the former. 9 parser cases plus 2 click cases in the controller test (the viewer is handed in as a recorder). The popup itself is for the Phase I2 guardrail run |
| I2.3 | `TerminalEnhancerFilterProvider : ConsoleFilterProvider` — the engine-independent floor | ❌ | Cut 2026-09-22 by decision 71: the epic targets the Reworked 2025 engine only, where the fold regions already work (decision 70) |
| I2.4 | The five built-in rules as plugin resources | ✅ | The five files landed with I1.2 (`resources/terminal/rules/`), loaded by `BlockDetector.builtInRules()`; the override by id came with I3.1 (`RuleCatalog.merge`: a user file with a built-in's id takes its place, so "edit a built-in" is "copy it and change it") |
...
- **I2.3 — Filter (cut by decision 71).** Was to be: `terminal/enhance/TerminalEnhancerFilterProvider.kt` registered `<consoleFilterProvider …/>` in `agenstorm-terminal.xml`, implementing plain `ConsoleFilterProvider` (the `ConsoleView` is null). Its `Filter` highlights the first line of a matched block and attaches a `HyperlinkInfo` opening the I2.2 viewer. **Always on, in every engine** — this is what makes the epic degrade rather than fail in the classic terminal or if I1.3 came back negative. Its per-line nature is a real constraint: the filter recognises a block *opener* and hands the offset to the viewer, which reads the surrounding text itself.
...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| I3.1 | Rule directory and the settings list | ✅ | 2026-09-22. `RuleCatalog` (pure: `load(folder, builtIns)` reads every `*.json` in name order, collects `RuleParseException`s, `merge` overrides by id; `active(disabledIds)`), `RuleRepository` (app service: `<config>/agenstorm/terminal-rules/`, created on first use — the first `rules` read — `reload`, `copyBuiltIn(id)` which never overwrites, `resetProjects` so open terminals re-fold), `EnhancerRulesTable` (`TableView`: id, source, On; edits a copy, `apply` writes `terminalEnhancerDisabledRules`), the three buttons on the settings page (`RevealFileAction.openDirectory`, reload, a chooser popup over the built-in ids). 5 + 5 + 2 cases in `RuleCatalogTest`, `RuleRepositoryTest`, `EnhancerRulesSettingsTest` |
| I3.2 | Hot reload; a broken file is skipped, never fatal | ✅ | 2026-09-22. `LocalFileSystem.addRootToWatch` on the folder plus a `BulkFileListener` on `VFS_CHANGES`, debounced 300 ms into a reload and a re-fold of every open project's terminals. `RuleFileNotice`: one WARNING balloon per (file, message) for the session, with Open Rules Folder; the same mistake saved again is not news, a different one is. A broken file is an entry in `errors`, the rest of the folder loads — pinned by `RuleRepositoryTest` |
| I3.3 | README section documenting the format | ✅ | 2026-09-22. *Terminal output that folds*: the five rules as a table, the field reference, two worked examples (a Laravel `dd` block, a PSR log line), the regex-only note; the intro paragraph and the features table; the changelog entry under Unreleased |
...
| Classic engine | With the engine set to Classic, a terminal tab gets no regions and the log shows no `com.pronskiy.agenstorm` frames — the feature is inert there, not broken (decision 71) | 🔲 | |
...
- **`ToolWindowManager.setMaximized(ToolWindow, Boolean)` and `isMaximized` are public and unannotated.** On a
  DOCKED or SLIDING tool window `setMaximized(tw, true)` expands it over the content area; `setMaximized(tw,
  false)` restores the previous proportion **by itself**, so the plugin stores no size. On FLOATING it
  maximizes the dialog and on WINDOWED the frame — a different meaning, hence the `ACTIVATE_ONLY` branch.
- **How far "maximized" reaches is the pane's geometry, not an argument we pass.** `ToolWindowPane` holds a
  `verticalSplitter` and a `horizontalSplitter` and nests one inside the other according to
  `UISettings.wideScreenSupport`: with it **off** (the default) the vertical splitter is outer —
  `top / (left | editor | right) / bottom` — so a bottom tool window spans the full width and the Project view
  sits *above* it, which is why maximizing the terminal takes the side tool windows' height with it. With
  widescreen **on**, `horizontalSplitter.setInnerComponent(verticalSplitter)` puts the terminal inside the
  editor's column and the side tool windows keep their full height. `setMaximized` merely calls the same
  `stretch` the splitter uses either way. Decision 41.
...
| J1.3 | Registration: action + binding in `plugin.xml`, title-bar button in `agenstorm-terminal.xml` | ✅ | **⌥⇧F12 on every platform since decision 45** — the macOS override is gone, and two tests ask the keymaps themselves whether anything else claims it and whether the binding reaches each keymap. | The `<reference ref="…"><add-to-group group-id="TerminalToolwindowActionGroup"/></reference>` split keeps the action itself in the core descriptor, so it loads (hidden) in IDEs without the Terminal plugin |
...
| J1.6 | `TerminalMaximizeShortcutPromoter`: ⌘⌥M goes to Maximize Terminal rather than Extract Method, and says so once | ✅ | Added after Roman pressed ⌘⌥M in the editor and got "Cannot perform refactoring". Decision 46: the candidates for the keystroke are ordered, nobody's keymap is edited. `com.intellij.actionPromoter` is public and unannotated; the promoter returns the whole list sorted, the way `WindowActionPromoter` does, so nothing is dropped from the dispatch |
| J1.4 | Tests | ✅ | 10 cases here plus 6 in `TerminalMaximizeShortcutPromoterTest`: registration and text, the binding is **non-empty**, each keymap gets the keystroke `plugin.xml` promises, the `$default` one is claimed by nothing else, and the macOS one is asserted to be *shared with `ExtractMethod`* — that case fails the day the platform stops sharing it, at which point the promoter can go (decisions 45, 46), the tool-window id pinned to `TerminalToolWindowFactory.TOOL_WINDOW_ID`, `update()` hidden with the feature off and without a project, and every row of the state table |
| J1.7 | The caret follows the maximize: focus asked for again once the pane has settled | ✅ | 2026-09-22, Roman: ⌘⌥M made the terminal big but typing stayed in the editor. `activate` asks for the focus before `setMaximized` reshapes the pane one event later; now the selected tab is focused once more after the reshape, through `ContentManager.requestFocus(content, true)` — the call the Terminal plugin makes for a tab it opens itself — under `IdeFocusManager.doWhenFocusSettlesDown`. A DEBUG trace (`com.pronskiy.agenstorm.terminal`) logs the focus owner at each step; the sandbox run shows the terminal's own editor (`EditorComponent file=null`) holding the focus after the reshape and the file's editor holding it after the second press. Roman, 2026-09-22: "looks good to me". Before the 1.8.1 cut the settled-callback became `Application.invokeLater(block, nonModal, project.disposed)`: `doWhenFocusSettlesDown` is deprecated in every overload and the verifier counted three new usages |
...
| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| The toggle | Terminal closed → shortcut opens it maximized, editor gone; press again → terminal hidden, caret back in the editor | ✅ | 2026-09-22 sandbox run on a demo project, four presses traced: maximized → focus in the terminal's editor, hidden → focus back in the file's editor. Roman: "looks good to me". Closed by J1.7 |
...
| Log | No `com.pronskiy.agenstorm` SEVERE/ERROR after the run | ✅ | Same pass. **Four `TransactionGuard` *Write-unsafe context* SEVEREs blaming Agenstorm** — three during the inline rename of `Notes.md`, one on the synthetic click-away create — all traced to the harness: robot scripts arrive through a plain `invokeAndWait`, which the platform treats as write-unsafe. Roman's own `runIde` log for 2026-09-14…16, real keystrokes including his rename sign-off, has **zero**, and the harness's Enter-driven creates and duplicates logged none. Recorded rather than changed on a guess. **Fixed 2026-09-16** (decision 62): the commit reaches the model through `Application.invokeLater`, and the same steps re-run — the rename of `Notes.md`, the rename of `Client.php`, a focus-loss create — logged **zero** Agenstorm-blamed SEVEREs; the one SEVERE in that run was the robot's own `saveAllDocuments`, and the log blames *Robot server* for it |
...
| K1.2 | `terminalEditorEnabled` + the customizer points `EDITOR`/`VISUAL` (plain **and** `_INTELLIJ_FORCE_SET_*`) at the shim, and captures the editor it displaced as the shim's fallback | ✅ | The fallback comes from `EnvironmentUtil.getEnvironmentMap()` (public, `@NotNull` its only annotation) — the login-shell environment is where a user's own `export EDITOR=…` lives — and a value pointing into our own bin directory is skipped, or a declined edit would loop back into the shim. `shimFor` now runs when **either** terminal feature is on, and the PATH entry is added only when an `open` name is actually shimmed: with just the bridge on, nothing shadows `open`. One consequence worth naming — `AGENSTORM_OPEN_PORT`/`_TOKEN` describe the endpoint, not the `open` feature, so they are now set whenever either shim is installed; Epic G's "off switch" guardrail should be read as both features off |
| K1.3 | `POST /edit` on `OpenRequestServer`: same token and body shape as `/open`, the answer held open until the IDE is done | ✅ | One `serve`/`dispatch` pair now takes the handler as an argument, so the two contexts differ only in what they call. `503` joins the vocabulary: the project closing under an open edit answers rather than leaving the shim on a socket nobody will write to |
| K1.4 | Open and wait: resolve the path, open it in the project that owns the terminal, save the document when the tab closes, answer then — and answer on project dispose so nothing hangs | ✅ | `EditCommandRouter` (pure, strict: one existing writable file, no options) + `TerminalEditSession` (subscribe, open, await the last close, save, **wait for the bytes**). The non-project-file bar does need `allowWriting`, and gets it. The flush barrier is decision 44 — the bug it fixes is the one this whole epic exists to prevent |
| K1.5 | Settings group of its own and the one-time balloon | ✅ | Its own `featureGroup`, for the reason J1.1 gives: the Terminal group's master toggle gates the `open` shim, and switching that off must not take `$EDITOR` with it. The balloon names the editor it displaced, which is the only place that answers "where did my vim go". One endpoint now serves both features, so `applyTerminalSettings` may unbind it only when **both** are off — switching the `open` shim off used to take the bridge's endpoint with it |
| K1.6 | README, CHANGELOG, CLAUDE.md inventory, and the correction to the YouTrack comment | ✅ (docs) / 🔲 (YouTrack) | README gains a section of its own and a Marketplace blurb; the CLAUDE.md inventory now covers Epics J and K, which it had not caught up with. The YouTrack comment is Roman's to post — the text is drafted and says what the `open -a` workaround actually does |
| K1.7 | The terminal steps aside when it is covering the editor, and comes back when the tab closes | ✅ | From Roman's first run: "when the terminal is maximized, I don't see the plan". The first version asked `ToolWindowManager.isMaximized` and did nothing in his second run — that flag is an identity check against a proportion `setMaximized(true)` records on the pane (`ToolWindowPaneState.isMaximized`), so it answers "did something call setMaximized", not "is the editor buried": a dragged splitter records nothing. It now **measures** instead — a docked, visible terminal taking more than 0.8 of the frame is covering the editor, however it got there. A maximized one is un-maximized (the inverse of Epic J's toggle); a merely tall one is hidden, since no public API moves a splitter. Coming back follows decision 42's rule, only what Agenstorm moved and only while it is still where we left it, and is `NonCancellable`. One line per edit records what the terminal looked like — DEBUG, since it fires on every edit and is something you go looking for: `#com.pronskiy.agenstorm.terminal.TerminalEditSession` in Debug Log Settings |
...
| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| The round trip | Ctrl+G in a Claude Code session opens the prompt in this window; editing, saving and closing the tab returns the **edited** text to the prompt | ✅ | Roman, 2026-09-10: "so it works" — observed on a plan, so the plan-mode row goes with it |
| Plan mode | Ctrl+G on a plan opens the plan file, and the edit comes back into the plan | ✅ | Same run: Ctrl+G on a plan opened the plan file and the edit came back |
...
| Terminal in the way | With the terminal maximized **and** with it merely dragged over most of the window, Ctrl+G shows the file, and closing the tab puts the terminal back as it was. Hiding or floating it while the file is open leaves that choice standing | ✅ | Roman, 2026-09-10: "cool it works". The log line from that run is the whole story — `visible=true docked=true maximized=false height=787 of 909 -> HIDDEN`: his terminal was **stretched, not maximized**, so the platform's flag said no while it covered 87 % of the frame. The first attempt failed on a real run: `isMaximized` was false, so nothing moved. A DEBUG line per edit says what the terminal looked like, should it ever need asking again |
| Off switch | Feature off → a new terminal has no `EDITOR`/`VISUAL` from us and no `agenstorm-edit` on disk | 🔲 | |
| Log | No `com.pronskiy.agenstorm` SEVERE/ERROR after the run | 🔄 | Zero `com.pronskiy.agenstorm` ERROR/SEVERE across the 2026-09-10 sandbox sessions. A daily-driver run is still what closes this |
...
- **The delay that governs is the *smart* one, not the plain one — this is what Roman's first run caught.**
  `startSmartFadeoutTimer(ms)` sets `mySmartFadeoutDelay` and starts **no alarm**; `BalloonImpl`'s own
  `AWTEventListener` (`BalloonImpl$1.eventDispatched`) starts it on the first AWT event with
  `if (mySmartFadeoutDelay > 0) { startFadeoutTimer(mySmartFadeoutDelay); mySmartFadeoutDelay = 0; }`. So a
  bare `startFadeoutTimer(ours)` is cancelled and replaced by the platform's 10 s / 5 min the moment the mouse
  moves — which is why the commit popup stayed. Set the smart delay, and start the plain timer only while
  `Application.isActive`, so a balloon that arrives while you are in another app still waits for you.
- **The commit result is the hardest case in the feature.** `ShowNotificationCommitResultHandler` builds a
  `CommitNotification` in `VcsNotifier.importantNotification()` — group **"Vcs Important Notifications"**,
  declared `STICKY_BALLOON`, so the delay it is fighting is 300 000 ms. It reaches the bus through
  `expirePreviousAndNotify(project)` → `Notification.notify(project)`, i.e. the **project** bus.
...
| L1.5 | Tests | ✅ | 16 cases across four classes. 8 in `AutoDismissPolicyTest`: the feature off leaves every type alone, information/warning/IDE-update get the delay, errors do not until opted in, a configured delay is honoured, and both ends of the range clamp. `NotificationAutoDismissListenerTest` pins both bus registrations and that a project-bus publish reaches a project-bus listener; `NotificationAutoDismissServiceTest` that the scheduled coroutine runs to completion rather than dying on its dispatcher; `NotificationAutoDismissRearmTest` the smart-delay regression |
| L1.8 | The remaining guardrails, automated as far as they go | ✅ | `applyDelay` takes `startNow` (defaulting to `Application.isActive`) so the background-pause branch is testable from both sides rather than only by eye. 20 tests in the package now. Three of the four open guardrails are pinned by test and want a confirming look; **Background pause is the one that still needs a running IDE** |
| L1.7 | **Fix: re-arm the smart delay, not the plain one** | ✅ | Roman's first run: "commit popup is still there". Three headless checks cleared the cheap explanations (policy, the project-bus registration, the coroutine running at all), and his session log showed the right build and a real commit at 23:31:21 with no plugin errors — so the fault was in which knob we set. `NotificationAutoDismissRearmTest` reproduces it against a real `BalloonImpl`: before the fix the smart delay was still 300 000. Now `applyDelay` sets `startSmartFadeoutTimer(ours)` and only starts the plain timer while the application is active |
...
| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| It hides | An information balloon disappears about 5 s after it appears | ✅ | 2026-09-11, Roman: "it works now, popup disappears after 5 seconds". The log shows two groups handled in that session — `Run 'composer install'` at 12:06:55 and `Vcs Important Notifications` at 12:07:13, each re-armed to 5000 ms on a `com.intellij.ui.BalloonImpl` within 17 ms of the publish |
| Nothing is lost | That same notification is still in the Notifications tool window afterwards | 🔄 | The mechanism is pinned by test: nothing on this path calls `expire()`, and `testTheNotificationIsNeverExpired` hides a real balloon and asserts the notification survives it. What is left is one look at the tool window |
| Background pause | Trigger a balloon, switch to another app for a minute, come back → it is still there, and *then* fades | 🔄 | **Deferred 2026-09-11 at Roman's call** — to be confirmed in a daily-driver run, the way Epic J's log guardrail was. The branch is pinned by a pair of tests (with `startNow = false` the smart delay is ours and no countdown runs; with `true` both are set), so what is unverified is only whether real app activation and AWT events behave the way those tests assume. The debug trace is left armed in the sandbox config, so the check costs one look at the log |
| Errors stay | An error balloon is still there after 30 s; switching "Also hide error notifications" on makes it go at 5 s | 🔄 | Pinned by test on both sides: `AutoDismissPolicyTest` for the policy and `testAnErrorNotificationIsNotScheduledAtAll` for the service refusing to schedule one. What is left is one look |
| Off switch | Feature off → balloons keep the IDE's own 10 s / 5 min behaviour, no restart | 🔄 | `testTurningTheFeatureOffTakesEffectWithoutARestart` schedules one, flips the setting and asserts the very next notification is already left to the platform — the setting is read per notification, so there is nothing to restart. What is left is one look |
| Log | No `com.pronskiy.agenstorm` SEVERE/ERROR after the run | ✅ | 2026-09-11 session: **zero `com.pronskiy` frames** anywhere. The one SEVERE is `XsltIconPatcher` asking `PsiManager` for an icon for a file under a scratchpad directory an earlier session had deleted — platform-only, nothing to do with this plugin |
...
- **Where the buttons are drawn depends on the IDE's settings, and the first build missed it.**
  `TextEditorWithPreview.isShowActionsInTabs()` is `ExperimentalUI.isNewUI() && UISettings.editorTabPlacement != 0`,
  so whenever editor tabs are visible the three actions are rendered as **tab actions** (`getTabActions()`) and the
  floating toolbar's group is wrapped in `ConditionalActionGroup { !isShowActionsInTabs() }` — leaving it empty.
  Detaching that toolbar therefore does nothing at all in the ordinary configuration. Decision 50.
- **Both paths read the same three ids.** `getTabActions()` → `createTabActions()` → `createViewActionGroup()`, and
  the floating toolbar takes the same group, so taking the actions covers every place they are shown.
- **The replacement must be a `ToggleAction`.** `getShowEditorAction()` does `ActionUtil.getAction(id) as ToggleAction`
  with a hard null check, so anything else in those slots throws while every split editor in the IDE is being built.

#### Phase M1 — Detaching the floating toolbar  ·  ❌ superseded by M2

| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| M1.1 | `LayoutSwitcherHider`: find the floating toolbar, detach it, put it back | ❌ | Built, shipped, and **did nothing on Roman's machine** — it detached a toolbar that `isShowActionsInTabs()` had already emptied. Its round-trip test did catch a real bug before release (decision 47, the `JLayeredPane` layer lost to Kotlin overload resolution), which is worth remembering: a green test on the wrong component is still a green test |
| M1.2 | Settings: `markdownHideLayoutSwitcher` (off) and the **Markdown editor** group | ✅ | Kept as is by M2; its own group rather than a row under *Markdown live markup*, since the two are independent. Default off because the buttons are the visible way to open the preview — the comment names Find Action as the way back |
| M1.3 | Wiring: `fileOpened` listener + startup activity | ❌ | Replaced by an `AppLifecycleListener`; the slots are application-level, not per editor |
| M1.4 | Tests | ❌ | Five cases against a real `LayoutActionsFloatingToolbar`. They passed throughout, because the component they exercised is real — it is just not the one the buttons come out of |
| M1.5 | Docs: README, changelog | ✅ | Unchanged by M2: what the feature does for the user is the same |

#### Phase M2 — Taking the three actions

| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| M2.1 | `LayoutSwitcherInstaller`: three `core/ActionSlot`s over `TextEditorWithPreview.Layout.*`, holding `HiddenInMarkdownToggleAction` | ✅ | Delegates everything to the displaced action and hides itself only when the file is Markdown, so `.editorconfig`, Swagger, Mermaid and the PHP eval scratch keep their buttons. The file is read from `PlatformCoreDataKeys.FILE_EDITOR` first and `CommonDataKeys.VIRTUAL_FILE` second, because a tab action and a toolbar action carry different data |
| M2.2 | Wiring: `LayoutSwitcherLifecycleListener` + `LayoutSwitcherUnloadListener` in `agenstorm-markdown.xml` | ✅ | `applicationListeners` in the optional config file, the `ScratchActionInstaller` shape. The settings page reaches it through `AgenstormSettingsListener`, since `core/` must not load a feature that needs the Markdown plugin |
| M2.3 | Tests | ✅ | 5 cases: the three platform actions exist and are `ToggleAction`s (the invariant that would break every split editor if wrong), the slot round trip gives back exactly what it took, the feature off leaves the platform's actions in place, the wrapper survives a missing action, and Markdown is told from PHP |
| M2.4 | `verifyPlugin`: still zero internal API | ✅ | 2026-09-13, PS-262.10315.130 and IU-262.10315.125: **Compatible on both, no internal-API section**, 58 experimental and 8 deprecated. Both moved by exactly the `DynamicPluginListener` default-method materialisation for one more implementor — `LayoutSwitcherUnloadListener`, `checkUnloadPlugin` ×2 and `beforePluginsLoaded`/`pluginsLoaded` ×4 |

**Exit guardrails — Epic M**

| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| It hides | Setting on → a Markdown editor shows no layout buttons | ✅ | **Roman, 2026-09-13: "works now"**, on the M2 build. The M1 build had failed this silently: he reported "hiding markdown buttons doesn't work" with the setting already `true` in `agenstorm.xml`, which is what sent the investigation to `isShowActionsInTabs()` |
| Markdown only | A non-Markdown split editor still shows its layout buttons | ✅ | Roman, 2026-09-21 sandbox pass with both settings off at launch, toggled on and off from the settings page with `conferences.md`, `.editorconfig` and a hand-moved Terminal open ("all good"); log clean. Earlier note: **Not exercised.** `.editorconfig` is the easy case to check in the demo project — it is the one bundled non-Markdown split-preview editor in PhpStorm. Pinned by `testANonMarkdownFileIsNotHidden`, but nobody has watched it |
| No restart | Toggling the setting takes effect on the editors that are already open, both ways | ✅ | Same pass, both directions on the open tab. Earlier note: not exercised; the setting was on before the run started and was never toggled during it. `ActionSlot` fires `CustomActionsListener.fireSchemaChanged()`, so toolbars and tabs re-read their groups |
| Reachable | Find Action → "Preview Only" still switches the layout while the buttons are hidden | 🔄 | Exercised in the same pass as step 3 of the checklist, under "either answer is a result"; Roman answered "all good" for the list as a whole, and what Find Action showed is not recorded separately yet — the README caveat stays until it is. Before that: **Skipped for now — Roman, 2026-09-16** (decision 60). Not exercised. Note the wrapper hides the action from **everywhere**, Find Action included, while a Markdown file is current — so the escape hatch the README promises may not hold. Worth checking before this is released |
| Log | No `com.pronskiy.agenstorm` SEVERE/ERROR after the run | ✅ | M2 run 2026-09-13 19:27: 2 081 lines, **zero `Plugin to blame` entries** and no `ActionSlot` warning, so all three ids were found and taken. The one SEVERE is a platform VFS-on-EDT assertion (`FSRecordsImpl.update` under `FileNavigator`) with no `com.pronskiy` frame in its 144-line stack |
| Verifier | Still zero internal API usages | ✅ | See M2.4 |
...
**1.5.0 (2026-09-14).** Epic M and Epic N ship together, both **off by default** — the first Agenstorm features that are. `patchChangelog` moved `[Unreleased]` into a `[1.5.0]` section; `build/distributions/agenstorm-1.5.0.zip` is 660 KB, sha256 `516d9b14…`. `verifyPlugin`: **Compatible** on PS-262.10315.130 and IU-262.10315.125, **zero internal usages**, 58 experimental and 8 deprecated — both counts up by exactly the `DynamicPluginListener` default-method materialisation for the two new unload listeners. Minor rather than patch because both add features. The README was reworked around a feature table and then run through an AI-writing check: spaced em dashes 28 → 2, bold-label bullets 38 → 6, bold spans 65 → 30, with the reference lists put back as plain bullets.
...
| Both ways, no restart | Switching the setting off puts back exactly the windows that were on the right, without a restart | ✅ | Same pass, on then off, no restart. Earlier note: **Not exercised.** `config/options/agenstorm.xml` came out of the run still reading `hideRightToolWindowBar="true"`, so the settings page was never applied during it. The reverse direction is the same `RightBarPlan.plan(…, clearRight = false)` the 9 unit tests cover, but nobody has watched it happen |
| Not greedy | A window that was on the left before the feature went on is still on the left after it goes off | ✅ | Same pass: the Terminal dragged left before the switch was still on the left after it went off. Earlier note: not exercised, for the same reason. Pinned by `never moves a window it did not move` and `stops owing a window the user moved somewhere of their own choosing` |
...
**Rename is taken with `ActionSlot`, like the other two** — decision 55, reversing the half of decision 52 that
planned to register on the `renameHandler` extension point. That point looked like the free fallback, and it is a
trap: `RenameHandlerRegistry.getRenameHandler` shows a **chooser dialog** as soon as two handlers claim the same
element, and the titles it groups them by are `toString()` for anything that is not a `TitledHandler`, so they never
collapse. In PhpStorm a directory is already claimed by `PlainDirectoryRenameHandler` and a `.md` file by Markdown's
own handler — joining that list would have turned one dialog into two.

**The editor is an overlay field, not a `TreeCellEditor`** — also decision 52, and the reason is that creation has no
row to edit.

**The field goes directly below the row that was clicked**, and only falls back to the target folder's last child when
there was no click — Roman, asked which row the overlay may cover: *"A virtual row should be inserted right below the
clicked row, otherwise if it's not triggered by click in the project tree, i think it should be last child."* Its
indent says where the element will land: one level in from the clicked row when the click was on the folder itself,
the same level when the click was on a file and the new one is its sibling.
...
- **Most rename handlers registered for the project view are pass-throughs, and two are not.**
  `PlainDirectoryRenameHandler.doRename` is `PsiElementRenameHandler.rename(...)` and Markdown's
  `MarkdownFileRenameHandler` is `RefactoringActionHandlerFactory.createRenameHandler().invoke(...)` — both exist only
  to force the chooser when something else also claims the element, so replacing them changes nothing but the dialog.
  `PhpDirectoryPackageRenameHandler`, `RenameProjectHandler`, `ProjectFolderRenameHandler` and `RenameModuleHandler`
  carry real work, and those elements are handed back. The test names none of them: `PlainDirectoryRenameHandler.isPlainDirectory`
  is public and static and answers exactly "does a package handler claim this directory".
- **`PsiElementRenameHandler.rename(element, project, context, editor, newName)` is the seam.** Public, unannotated,
  and with a non-null name it builds the Rename dialog and disposes it without showing it — so the element's own
  `RenamePsiElementProcessor` still runs and `BaseRefactoringProcessor` still wraps the whole thing in one command.
- **The creation primitives are public.** `CreateFileAction.MkDirs(newName, directory)` (public static final, no
  ...
  `createSubdirectory` / `checkCreateFile` are clean. `FileTemplateManager` and `FileTemplateUtil.createFromTemplate`
  are clean too and were built against before decision 54 dropped them.
...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| O1.0 | `verifyPlugin` fails the build on internal API, instead of only reporting it | ✅ | Found while planning the epic: `failureLevel` in `build.gradle.kts` listed `COMPATIBILITY_PROBLEMS`, `INVALID_PLUGIN` and `MISSING_DEPENDENCIES` but **not** `INTERNAL_API_USAGES`, and the comment above it still justified tolerating internal API by naming `scratchLanguageFilter`, gone since 1.2. So §2's gate has been a person reading a 259 KB HTML report since 2026-09-10. It is now the build's |
| O1.1 | `InlineNamePolicy`: validation, path splitting, initial selection range | ✅ | The one piece of pure logic in the epic and the only part testable in full, so it is kept clear of the platform — typed text plus the sibling names in, a verdict out. 20 cases in `InlineNamePolicyTest`. Names are validated **strict** (`PathUtilRt.isValidFileName(name, true)`), which rejects `<>:"\|?*;` and control characters: a name typed on macOS then has to work for whoever clones the repo on Windows. Collisions are only checked for a name with no folders in it — what sits inside `src/Http` is not something the field knows, and creation reports it |
| O1.2 | `ProjectTreeAccess` + `InlineNameEditor` + `InlinePlaceholder` + `InlineRowSession`: the field and the real row it sits on | ✅ | `ProjectTreeAccess` is the only file that names a project-view type, so the platform surface is one import list. The field is added as a child of the tree and positioned absolutely, which is how Swing mounts its own editing container; it scrolls with the tree because it is in it. **For a new element the row is a real, temporary node** (decision 61), so the tree moves everything below it down. Two earlier versions covered the row beneath instead — the plain overlay, then a taller-row spacer the fixed-height tree ignored (decision 58) — and Roman's screenshots caught both. Geometry comes out of a pure function — rectangles in, a rectangle out — so the placement rule above is testable without a tree. Height always comes from a `Rectangle`: IntelliJ trees run variable-height and `getRowHeight()` returns 0 |
| O1.3 | `InlineCreateAction` + `ProjectViewActionInstaller` + the two lifecycle listeners | ✅ | One action class serving both slots, told apart by the `InlineNameKind` it is built with. Commit is one `WriteCommandAction` with one group id, so the folders and the file a `src/Http/Client.php` made undo together. **The new file is empty**, which is what the stock action does — decision 54 |
| O1.4 | `InlineRenameAction` in the `RenameElement` slot, and `InlineRename`'s gate | ✅ | Not the `renameHandler` EP — decision 55. The gate: feature on, the event came from the project tree, one selected `PsiFile`, or a `PsiDirectory` that is plain, not a content root and not the project's own folder. Everything else goes to the displaced action, so Shift+F6 in the editor and on a PSR namespace folder is untouched |
| O1.5 | Settings: `projectTreeInlineNamingEnabled` (on) and the **Project tree** group; tests | ✅ | On by default, unlike Epics M and N: this replaces a dialog with a field rather than removing something from the IDE, and the dialog is one setting away. 44 cases across five classes: 20 in `InlineNamePolicyTest`, 9 in `InlineRowGeometryTest`, 5 in `ProjectViewActionInstallerTest` (the slot round trip, sync following the setting, a second take changing nothing, a null delegate not throwing), 8 in `InlineCreateTest` (nested paths, a trailing separator asking the file field for a folder, an empty file per decision 54, a clash reported rather than thrown), 6 in `InlineRenameTest` (the gate: a plain file and a Markdown file yes, a content root and a PHP class element no), plus the settings round trip |
| O1.6 | Docs: README, changelog, this epic, decisions 52–55 | ✅ | Its own README section with the trades named — the empty file, the dialogs that stay — a feature-table row, and an `[Unreleased]` bullet that also records the verifier gate of O1.0. The Marketplace description block gains one short paragraph |
| O1.7 | `verifyPlugin`: still zero internal API | ✅ | **The gate, and it passes — this time as the build's own, not a report's** (O1.0). 2026-09-14 against PS-262.10315.130 and IU-262.10315.125: **Compatible on both, no internal-API section**, 62 experimental and 10 deprecated. Both counts moved by exactly the known amount for one more `DynamicPluginListener` implementor: `checkUnloadPlugin` deprecated ×2, `beforePluginsLoaded` / `pluginsLoaded` experimental ×4. **It also caught two things first** — `AnAction.actionPerformed` is `@ApiStatus.OverrideOnly`, so the fallback goes through `ActionUtil.performAction`; and both `runReadAction { }` (the `ActionsKt` extension) and `ReadAction.compute` are deprecated in 262, so the two lookups simply run on the EDT, which already holds read access |

#### Phase O2 — Duplicate

| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| O2.1 | `CopyNameSuggester`: the name the field opens with | ✅ | `Client.php` → `Client 2.php`, and `Client 2.php` → `Client 3.php` rather than `Client 2 2.php`. The extension is kept out of the counted part by the same "last dot, but not a leading one" rule the rename selection uses, so `archive.tar.gz` becomes `archive.tar 2.gz`. 8 cases |
| O2.2 | `InlineDuplicate`: the copy | ✅ | `CopyFilesOrDirectoriesHandler.copyToDirectory`, the same call the Copy File dialog ends in, so a folder comes with everything in it. One command group id, so a copied folder and its contents undo together. 3 cases |
| O2.3 | `InlineDuplicateAction`, its own action rather than a slot | ✅ | Decision 57. Registered in `plugin.xml` into `ProjectViewPopupMenu` after `CopyElement`, with **no default shortcut** |
| O2.4 | Docs: README, changelog | ✅ | Two bullets in the Project tree section, one naming the trade — no shortcut out of the box — and one saying plainly that F5 and ⌘C/⌘V are untouched |

#### Phase O3 — The New menu's typed entries

| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| O3.1 | The scan: which New entries the feature takes | ✅ | Walks `NewGroup` down to depth 3 — the typed entries are not its direct children; JavaScript and CSS add themselves to `NewWebDevelopment` — and takes every non-group entry whose name ends in ` File`. In PhpStorm 2026.2 that is seven: `PhpNewFile` and `PhpSimpleNewFile` (*PHP File*), `NewJavaScriptFile`, `NewTypeScriptFile`, `NewHtmlFile`, `NewComposerJsonFile` and `CreateEditorConfigFile`. Enumeration goes through `DefaultActionGroup.getChildren(ActionManager)`, public, final and unannotated — **`ActionGroup.getChildren(AnActionEvent)` is `@ApiStatus.OverrideOnly` and the verifier caught the first version using it** |
...
**1.6.0 (2026-09-16), released the same day.** Epic O ships **on by default**, with the O1.0 verifier gate. `patchChangelog` moved `[Unreleased]` into a `[1.6.0]` section; `build/distributions/agenstorm-1.6.0.zip` is 746 KB, sha256 `8888ae5f…`. `verifyPlugin`: **Compatible** on PS-262.10315.130 and IU-262.10315.125, **zero internal and zero override-only usages** — now failing the build rather than filing a report — 65 experimental and 10 deprecated. The experimental count is up by exactly `NodeSortOrder` and `NodeSortSettings` for the placeholder's sort pinning, plus the `DynamicPluginListener` default-method materialisation for one more unload listener. Minor rather than patch because it adds a feature. **Carried knowingly:** the two 🔄 guardrails need a person — a click away with a real mouse and a real dynamic unload — and their code paths passed in the guardrail pass.

**Exit guardrails — Epic O**

| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| New file | Alt+Insert → File on a folder puts an editable row under it; `Client.php` + Enter creates an empty file there and opens it | ✅ | Guardrail pass 2026-09-16, driven from inside a `runIdeForUiTests` IDE through the robot server's JS endpoint — actions invoked directly, tree rows and files read back, window-only renders; no OS keyboard or mouse, since the machine was in use. New File on `docs/shmest.md`: the placeholder row appeared at y=90 directly under it and `empty` moved from y=90 to y=112; Enter made a 0-byte `docs/Api.php`, opened it, selected it, and the placeholder was gone. Repeated on `empty/`, which **found a defect** — an empty folder is a leaf, `expandPath` does nothing on a leaf, the row never showed and the stock popup opened after 2 s — fixed the same day and re-verified. **Roman, 2026-09-16: "works now"** on the row insertion |
| New folder | The same for Directory | ✅ | Same pass. New Directory on `src`: the row was its first child at y=178, `Http` moved down, Enter made `src/Middleware` |
| Rename | Shift+F6 on `Client.php` in the tree edits in place with `Client` selected, not `Client.php`; a usage in another file is updated; one Undo puts both back | ✅ | **Roman, 2026-09-14: "rename works now"**, after decision 56 took the usage preview off. The first run had it opening the refactoring in a tool window instead — "i thought it would be hidden by default" — which is what found the preview flag. The undo half is not separately confirmed; `InlineRenameTest` pins that the `require_once` follows the rename. **Guardrail pass 2026-09-16:** `docs/Notes.md` opened with `Notes` selected and became `Guide.md` with no dialog. On `src/Client.php` the pass stopped at PhpStorm's `AutomaticRenamingDialog` — the file holds a class of the same name, and asking whether to rename it too is the follow-up decision 53 kept — which a robot cannot answer without real input. **Changed 2026-09-16** (decision 62): the question is answered as OK would, and the same rename in the test IDE went through with no dialog — class `HttpClient`, the `require_once`, the return type and `new HttpClient()` all updated, one undo step |
| Nested | `src/Http/Client.php` creates the two folders and the file | ✅ | Same pass. `Http2/Json/Codec.php` typed on `src` made both folders and the file |
| Cancel | Esc leaves nothing behind — no file, no folder, no VFS event | ✅ | Same pass. New File on `docs/Notes.md`, typed `Nope.php`, Escape: nothing on disk, no placeholder left in the tree, no popup |
| Click-away | Clicking another row commits what was typed | 🔄 | Same pass, **synthetic only**: a non-temporary `FOCUS_LOST` delivered to the field's listeners committed `Away.md`. A real click away needs real focus, which the pass deliberately did not take — **not exercised with a mouse** |
| Fallback | Shift+F6 on a module root still opens the platform's Rename dialog | ✅ | Same pass. Shift+F6 on the project root opened the platform's own *What would you like to do? Rename directory / Rename project* chooser, not the row |
| Typed entries | New → PHP File / HTML File / JavaScript File opens the row, not a dialog, and the file arrives with its template body | ✅ | Same pass. New → PHP File on `docs/Notes.md` opened on `.php`, caret 0, nothing selected, directly under the clicked file; Enter with `Model.php` made a file starting `<?php`. **Roman, 2026-09-16: "works now"** on the same flow |
| Typed fallback | New → composer.json File and EditorConfig File still open their own dialogs, because no template answers to their name | ✅ | Same pass. New → composer.json File has no dialog in the first place — the stock action writes a fixed-name `composer.json` — and with no template of its entry's name the wrapper handed it straight back: `docs/composer.json` appeared exactly as it always has |
| Duplicate | Right-click a file → Duplicate: a row appears below it reading `Client 2.php` with `Client 2` selected; Enter makes the copy next to the original | ✅ | Same pass. On `src/Client.php` the row opened on `Client 2.php` with `Client 2` selected, directly under the original; Enter made a byte-identical copy |
| Duplicate a folder | The same on `Http` copies the folder with `Request.php` and `Response.php` in it, and one Undo removes all of it | ✅ | Same pass. On `src/Http` the row opened on `Http 2` and the copy held `Request.php` and `Response.php`; one Undo — *Undo Duplicate 'Http'* — removed all of it with no confirmation. The row sits after the expanded folder's children, the nearest place a sibling can go |
| F5 is untouched | Copy File… still opens its dialog with the target directory browser | ✅ | Same pass. `CopyElement` is the platform's `CopyElementAction` with the feature on, off, and after unload |
...
| Unload | Disabling the plugin puts the stock `NewFile` and `NewDir` actions back | 🔄 | Same pass, through `ProjectViewUnloadListener.beforePluginUnload` with a field open: the stock actions came back, the placeholder was cleared and the field closed. **A real dynamic unload was not performed** |
...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| P1.1 | `ProjectTabsPanel` sizes: `getPreferredSize` = the FULL plan (natural widths, `tabsMaxWidth` cap, no window cap); `getMinimumSize` = the COMPACT plan (every tab 32 px); `availableWidthProvider`, `availableWidthFor` and the half-window rule deleted; `doLayout` picks the mode from `width` alone | ✅ | 2026-09-19. `fullWidth()` / `compactWidth()` over the one `total(widths, more)` helper; `plan(width)` from `doLayout`, nothing reads the parent or the window. `ProjectTabsOverflowTest` +1 (`testMinimumIsTheIconStripAndPreferredIsTheWholeStrip`: twelve tabs, minimum < preferred, both content-driven and unchanged by a COMPACT layout), the half-window case dropped, the others driven by `setSize` alone. `check` green |
| P1.2 | SHRUNK mode between FULL and COMPACT: the widest tabs are cut first, water-filling, so every shrunk tab ends at the same width while short names keep theirs — continuous as the window narrows, without the jump Chrome's equal widths would give; floor `MIN_WIDTH` (72); the panel places labels at the shrunk width, `JBLabel` ellipsizes the name, the × stays | ✅ | 2026-09-19. `shrinkLevel(natural, budget)`: start at `budget / n`, fix every tab no wider than the level, raise the level for the rest, stop when a pass changes nothing; null below `MIN_WIDTH × n`, which hands over to COMPACT. `Plan` carries per-tab widths and `place(component, x, width)` uses them; `isCompact` is only set for COMPACT/OVERFLOW. `ProjectTabsOverflowTest` +2 (widest cut first while a one-letter name keeps its width and the strip fills what it was given; six long names end equal, sit at exactly 72 on the floor and go icon-only one pixel below) |
| P1.3 | Chevron removed: `Mode.OVERFLOW`, `moreButton`, `hiddenProjects`, `onOverflow`, `ProjectTabActions.showOverflowPopup` / `overflowGroup`, `tabs.more.tooltip`. Below the minimum the toolbar clips the strip, which the P2 cap keeps out of reach | ✅ | 2026-09-19. All gone, plus the `visible` bookkeeping in `doLayout`, `paint` and `insertionIndex` that only the chevron needed; `Plan` is now `(mode, width, widths)` and `plan()` is three lines: FULL if it fits, SHRUNK if a level ≥ 72 exists, COMPACT otherwise. `ProjectTabsOverflowTest`: the chevron and popup cases replaced by one — one pixel below the icon strip every tab is still there, icon-only, and the strip lays out exactly its minimum |
| P1.4 | `ProjectTabsOverflowTest` → `ProjectTabsLayoutTest`: FULL / SHRUNK / COMPACT chosen by width; minimum < preferred; shrunk tabs equal and ≥ 72; short names untouched; no chevron at any width. README bullet + CHANGELOG | ✅ | 2026-09-19. Renamed with `git mv`; six cases, all driven by `setSize` + `doLayout`. README: the "narrow toolbar" bullet now describes shrink → icons and says nothing hides. CHANGELOG: a *Changed* entry under Unreleased. CLAUDE.md: the `ui/` inventory names the three modes and the minimum-below-preferred rule. `check` green |
...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| P2.1 | `tabs/ProjectTab.kt`: `sealed class ProjectTab(key, name)` — `Loaded(project)` / `Offloaded(key, name, sinceMs)`. `ProjectTabsModel.State` gains `offloaded: MutableList<OffloadedEntry>` (key, name, since) and `lastActive: MutableMap<String, Long>`; `tabs(): List<ProjectTab>` merges both by stored order; `markOffloaded(project, now)` / `unmarkOffloaded(key)` / `forget(key)` / `touch(key, now)` / `lastActive(key)`; `projectOpened` drops the offloaded entry and touches; a user close needs nothing — not open and not in `offloaded` is gone; at most `MAX_OFFLOADED` (12) entries, the oldest forgotten first. `Listener.tabsChanged(List<ProjectTab>)` | ✅ | 2026-09-19. `tabs/ProjectTab.kt` (sealed: `Loaded(project)` / `Offloaded(key, name, sinceMs)`); `State.offloaded` is a list of `OffloadedEntry` beans, `State.lastActive` a map, both round-trip through `XmlSerializer`; `remember` also trims `lastActive` to keys still in `order`. `moveTab` takes a key so offloaded tabs can be dragged. Until P2.7 the strip, the actions, the navigation actions and `NativeTabStrip` go through the new `loadedProjects()`, so nothing visible changes yet. `ProjectTabsModelTest` +6 (stored order across both kinds and the entry going on `projectOpened`; unmark/forget with listeners seeing `ProjectTab`s; the cap of 12, oldest first; touch; serialization; move by key). The `projectOpened(project, now)` default parameter tripped the CLAUDE.md pitfall — `compileTestKotlin --rerun-tasks` once — then `check` green |
| P2.2 | `tabs/offload/TabsFrameListener : FrameStateListener` (`applicationListeners`) → `touch` on `onFrameActivated` | ✅ | 2026-09-19. `FrameStateListener` verified by `javap`: all four methods are `default`, `TOPIC` public. The listener ignores a frame without a project and a disposed or default one; its `clock` parameter (defaulted, so the no-arg constructor the registry needs still exists) is for the tests. `TabsFrameListenerTest` (2) over an `IdeFrame` proxy. `check` green |
| P2.3 | `tabs/offload/OffloadPolicy`, pure: `(loaded: List<Candidate(key, lastActive, busy)>, activeKey, now, idleMs, maxLoaded) → List<String>` — idle ones first, then the least recently active while `loaded − chosen > maxLoaded`; never the active project, never a busy one, never the last loaded project | ✅ | 2026-09-19. `OffloadPolicy.choose(candidates, activeKey, now, idleMs, maxLoaded, enabled)` → keys oldest-activity-first; two passes over the eligible (not active, not busy) candidates sorted by activity, both stopping when one project would be left, the second also at the cap (`coerceAtLeast(1)`). No activity recorded = active now. `OffloadPolicyTest` (10, plain `TestCase`): oldest first, the threshold is inclusive, active/busy/last stay, cap takes the least recently active, idle ones count towards the cap, cap 0 keeps one, null activity, disabled |
| P2.4 | `OffloadGuard` EP `com.pronskiy.agenstorm.offloadGuard` (`busyReason(project): String?`, dynamic, the `commitContextProvider` shape): `RunningProcessesGuard` (`plugin.xml`, `ExecutionManager.getRunningProcesses`), `TerminalCommandGuard` (`agenstorm-terminal.xml`, `TerminalWidget.isCommandRunning()` + the classic `hasRunningCommands()`). The guards are the whole reason no dialog can appear | ✅ | 2026-09-19. Verified by `javap`: `ExecutionManager.getRunningProcesses(): ProcessHandler[]` (abstract, public), `TerminalToolWindowManager.getTerminalWidgets(): Set<TerminalWidget>` (public), `TerminalWidget.isCommandRunning()` (a `default` method in `intellij.platform.execution.impl.jar`, so the interface is the platform's) and `ShellTerminalWidget.asShellJediTermWidget` / `hasRunningCommands()` (public, `throws IllegalStateException`, caught and logged at DEBUG as idle). `OffloadGuard.busyReason(project)` on the companion asks every extension and returns the first reason; the two bundle keys are user-facing (`tabs.offload.busy.terminal` / `.process`). Both guards take an injectable source with a default, so the EP's no-arg constructor exists. `OffloadGuardTest` (3): both guards resolve in the test IDE and an idle project is not busy; a started `NopProcessHandler` is busy until destroyed; a `TerminalWidget` proxy answering `isCommandRunning`. `check` green |
| P2.5 | `tabs/offload/ProjectOffloadService` (`@Service(APP)`, `class(scope: CoroutineScope)`): a 60 s loop started from `TabsStartupActivity`, plus `onProjectOpened` for the cap; runs the policy on `Dispatchers.EDT` under `ModalityState.nonModal()`; per key: `markOffloaded` → `closeAndDispose` → `unmarkOffloaded` when it returns `false`. First offload ever → one balloon (`projectsOffloadNoticeShown`: which project, why, "click its tab to load it again", a Settings link). INFO per offload with its reason (idle / cap), DEBUG per skip with the guard's reason | ✅ | 2026-09-19. Split in two: `Offloader` holds the sweep with every collaborator a function (settings, active key, guard, close, clock, notice), `ProjectOffloadService` wires the real ones — `IdeFocusManager.getGlobalInstance().lastFocusedFrame?.project` for the active key, `ProjectManager.closeAndDispose` for the close — and runs it from its injected scope: a `while (isActive) { delay(60 s); sweep() }` loop started once, plus one sweep 5 s after every open for the cap, both `withContext(Dispatchers.EDT + ModalityState.nonModal().asContextElement())`. `OffloadNotice.showOnce` follows `TerminalOpenNotice` (flag written before the balloon). Settings fields `projectsOffloadEnabled` / `projectsOffloadAfterMinutes` / `projectsMaxLoaded` / `projectsOffloadNoticeShown` added now, their UI in P2.8. Found on the way: `remember` wiped every `lastActive` entry whose key was not yet in `order`; it now forgets an activity only with its key. `OffloaderTest` (5): idle ones closed and kept as tabs with the notice per project; a refused close takes the mark back; a guard objecting right before the close skips; the cap reason; disabled. **Seen in the IDE 2026-09-19:** `Dispatchers.EDT` satisfies `closeAndDispose`'s `assertWriteIntentReadAccess` — seven offloads in one session, no write-intent line in the log |
| P2.6 | Load: `ProjectTabActions.load(tab)` → `scope.launch { ProjectUtil.openOrImportAsync(Path.of(key), OpenProjectTask.build { forceOpenInNewFrame = true }) }`; `null` or a missing directory → balloon + `forget`. A project opened any other way ("+", the recent list, `open`, the CLI) that has an offloaded tab turns it solid in place through `projectOpened` | ✅ | 2026-09-19. Verified by `javap`: `OpenProjectTask.build()` (static) + `withForceOpenInNewFrame(true)`, `ProjectUtil.openOrImportAsync(Path, OpenProjectTask)` (suspend, in `intellij.platform.ide.impl.jar`; `OpenProjectTask` lives in `intellij.platform.usageView.jar`). `ProjectLoader(model, open, exists, notify)` with real defaults; `ProjectOffloadService.load(tab)` launches it. **One deviation from the step text:** an open that returns `null` keeps the tab rather than forgetting it — a declined trust dialog should not cost the bookmark; only a directory that is gone forgets it. `ProjectLoaderTest` (3) with a fake opener. The click itself is wired in P2.7 |
| P2.7 | UI: `ProjectTabLabel(tab: ProjectTab, …)`; offloaded → a 1 px dotted rounded border in the inactive text colour, the icon through `IconLoader.getTransparentIcon(icon, 0.5f)`, the name in the inactive colour, tooltip "path — offloaded 2 h ago, click to load", × and middle click = forget. Context menu: loaded → Close, Close Others, **Offload Project** (respects the guards; busy → balloon with the reason), Copy Path; offloaded → Load, Forget, Copy Path. Drag-reorder works for both; Next/Prev/Close tab and `neighbourOf` see loaded tabs only | ✅ | 2026-09-19. `ProjectTabLabel(tab: ProjectTab, …)` with `project: Project?` for the callers that only deal with windows, `isOffloaded`, `isLoading` (icon back to full alpha while the project opens; the strip rebuild that follows `projectOpened`, or `model.refresh()` on a failed open, clears it), `textColor()` internal for the test, `projectIcon(key, name)`. The dotted border is a `BasicStroke` dash of 2 on / 2 off in `NamedColorUtil.getInactiveTextColor()`, drawn on the same rounded rectangle the fills use; tooltip through `DateFormatUtil.formatBetweenDates` (verified by `javap`, as was `IconLoader.getTransparentIcon(Icon, float)`). Panel callbacks carry `ProjectTab`; `ProjectTabActions.wire` branches on the kind — `load(tab, panel)` / `ProjectOffloadService.load`, `forget`, `moveTab(key)` — and `contextMenuGroup(tab, owner, tabs)` builds the loaded menu (Close, Close Others, **Offload Project**, Copy Path) or the bookmark one (Load Project, Forget, Copy Path). `Offloader.offload(project)` shares `closeMarked` with the sweep and returns the busy reason; `ProjectOffloadService.offloadNow` shows it in a balloon. Tests: `ProjectTabLabelTest` +2, `ProjectTabsPanelTest` +1, `ProjectTabActionsTest` +1 and the menu case updated, `OffloaderTest` +1; the layout and reorder tests moved to `ProjectTab.Loaded`. `check` green |
| P2.8 | Settings: `projectsOffloadEnabled` (true), `projectsOffloadAfterMinutes` (120, 5–1440), `projectsMaxLoaded` (8, 1–50), `projectsOffloadNoticeShown`; three rows in the Project tabs group; apply re-runs the policy | ✅ | 2026-09-19. Rows named `tabs.offload.enabled` / `tabs.offload.minutes` / `tabs.offload.maxLoaded`, `intTextField(5..1440, 5)` and `(1..50, 1)`, each with its own `onApply { ProjectOffloadService.getInstance().sweepSoon() }` (per-cell, as E2.4 found the DSL needs). The comments say what offloading is and that a running command or process is never offloaded. `TabsSettingsPanelTest` +1. `check` green |
| P2.9 | Docs: README (the "narrow toolbar" bullet rewritten; an "Offloaded projects" subsection; the options paragraph), CHANGELOG (two Added entries, with the two trades as sub-bullets: an offloaded project is not reopened by the IDE at launch but comes back as a dotted tab; a running command or process keeps a project loaded), CLAUDE.md inventory (`tabs/offload/`, the EP, the settings) | ✅ | 2026-09-19. README: a bullet in the list, an *Offloaded projects* subsection with the two things that never get offloaded and the restart note as a quote, the options paragraph extended. CHANGELOG: an *Added* entry above the P1 *Changed* one, its three sub-bullets the trades. CLAUDE.md: `ProjectTab` on the model line and an `offload/` entry naming every class, the EP and the settings |
...
| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| Idle | A project left in the background turns dotted within ~6 minutes, its memory is released, the balloon shows once, the active project is untouched | ✅ | Roman, 2026-09-19 sandbox run with the pre-set 5 min / cap 2 ("all good, but let's make the dotted border a more transparent so that it's not too distracting") — the border is now the inactive colour at 40 % alpha (`BORDER_ALPHA`, `ProjectTabLabelTest` +1, commit `7869956`) |
| Cap | Opening a third project offloads the least recently active of the other two | ✅ | Same run. The log shows the path at work: seven `Offloaded <name> (CAP)` INFO lines between 22:29:49 and 22:30:25 as the sandbox reopened its projects and Roman loaded bookmarks back, none refused |
| Terminal guard | `sleep 600` in the idle project's terminal keeps it loaded, the DEBUG log says why; after Ctrl+C it goes within a minute | ✅ | Same run |
| Process guard | A running PHP script keeps its project loaded and **no dialog ever appears** | ✅ | Same run |
| Load | Clicking a dotted tab opens the project in the same position, in the same window group, and the tab is solid again | ✅ | Same run. **Found right after (Roman): closing the last loaded project with only bookmarks left gave no window at all** — `close` only ever looked for a *loaded* neighbour. Fixed the same evening (commit `5f7b9bf`): `neighbourTabOf` takes the nearest loaded tab, else the nearest bookmark, which `ProjectLoader.loadThenClose` loads first and closes the old project once the new one is open. `ProjectTabActionsTest` +1, `ProjectLoaderTest` +1. **Checked by Roman the same evening: "works now."** One exception in that session's log, not ours: `AlreadyDisposedException: Container class com.intellij.tailwind.lsp.TailwindLspServerService is already disposed`, thrown inside the Tailwind plugin's LSP client when the language server's `projectsDestroyed` notification arrived on its reader thread after the project container was gone — at 22:42:21 the cap had just offloaded `happy` and the server was stopped normally. No `Plugin to blame`, no plugin frames; any project close with a Tailwind server running can race the same way. Nothing to do on our side |
| Forget | × on a dotted tab removes it; the project is still under "+" | ✅ | Same run |
| Restart | Dotted tabs survive a restart as dotted; the IDE reopens only the loaded projects | ✅ | Same run |
| Off switch | Feature off: nothing is offloaded any more; existing dotted tabs stay and still load on click | ✅ | Same run |
| Log clean + verifier | No plugin exceptions; `verifyPlugin` Compatible with zero internal usages, the experimental count moved by `isCommandRunning` only | ✅ | Session 2026-09-19 22:29:16–22:30:47: zero SEVERE/ERROR, **zero write-intent or TransactionGuard lines** — `Dispatchers.EDT` satisfies `closeAndDispose` (the P2.5 open point); the only WARNs are the platform's `GitRepositoriesHolder`. Verifier 2026-09-19 22:36: **Compatible on PS-262.10968.76 and IU-262.10968.63, no internal-API section**, 10 deprecated (unchanged), experimental 65 → 76, all eleven from this epic's code: `HoverListener` (the class, its constructor, `addTo` and the three overridden methods — ten lines, the hover fix) and `TerminalWidget.isCommandRunning` (one) |
...
| Q2.2 | Controller: the TABLE reveal rule; coexistence with the plugin's table region; persisted-state restore; off switches | ✅ | One branch in `isRevealed`: a TABLE region is revealed for `TextRange(start + 1, end)` — past its first character or at its end, or a selection over it. `TableFoldingTest`, eight cases; six of them passed before the branch existed, which is the platform confirming the epic's bets: after the folding pass the plugin's table region lies inside ours and ours stays collapsed, a caret moved into the table by `moveToOffset` opens it, a session-restored region on the table's range is replaced |
| Q2.3 | `TableInlays` + `TableInlayRenderer` + `TablePainter`: one block inlay per collapsed table, width and scheme updates | ✅ | `TableInlaysTest`, seven cases: the inlay hangs off the region's start with `showAbove(false)` and `showWhenFolded(true)` and **has bounds while the table is folded** — the placement bet, confirmed; it goes on reveal, returns on collapse, follows a foreign expand undone by the policy, and leaves with the controller. An edit updates the inlay in place (`update()`), it is not recreated; a model with the same content keeps its cached geometry, so a keystroke elsewhere costs no layout. Width changes coalesce through `invokeLater` — `Alarm` carries internal members and was left alone. Rules: header rule full colour, body rules at 55 % alpha, no vertical lines; code spans on the fence-card background |
| Q2.4 | Click and hover: reveal at the cell, follow links, hand cursor; `LinkDestinations` shared with the goto handler | ✅ | `TableClickTest`, five cases, driven through `clickTable(point)` / `tableLinkAt(point)` the way the checkbox tests drive `toggleCheckbox`. `LinkDestinations` delegates to the goto handler's `targetsOf` rather than moving it, so its tests stay put; URLs and `mailto:` go to the browser directly instead of through the `WebReference` target, which is what keeps the test hook and a bare autolink on one path. A file link opens the file (asserted through `FileEditorManager`) |
| Q2.5 | Settings row `markdown.tables`, bundle keys, README, CHANGELOG | ✅ | "Render tables in place (click a cell to edit)" after the thematic-breaks row; `MarkdownSettingsPanelTest` has its case; README pitch, feature bullet and CHANGELOG `[Unreleased]` written. `pluginVersion` stays 1.7.0 until the cut |
...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| Q3.1 | Spike: a field over one cell of the block inlay — placement, scroll, resize, focus in and out, Esc; child of the content component or a popup | ✅ | Answered without a throwaway. Placement and scroll by construction: `EditorComponentImpl` is a `JTextComponent`, so it has no layout manager and a child keeps its bounds, and it is the component that scrolls, so a child at the cell's bounds moves with the text and nothing has to follow the viewport; only a re-layout (resize) moves the cell, and `reposition` runs after every sync and policy pass. Verified headless in `CellEditorTest`: the field is a child of the content component inside the inlay's bounds. Focus in and out and Esc giving focus back are the Q3 run's to judge — `EditorEmbeddedComponentManager` is in no jar of this build, so the popup was never needed |
| Q3.2 | `CellEditor`: a plain click opens the clicked cell; Enter or focus loss commits in one command; Esc cancels; the table re-renders; missing cells appended | ✅ | `markdown/tables/CellEditor.kt`, driven by `clickTable` for a non-link hit. The cell's range is held in a `RangeMarker`; commit writes through `WriteCommandAction` under one group id, then commits the document and calls `syncNow`, so the table is rendered again before the click handler returns — which is also what lets Tab open the next cell from fresh geometry. A ragged row without the cell gets ` text \|` appended (or ` \| text` when the row has no closing pipe). `CellEditorTest`, nine cases, including one Undo reverting one cell |
| Q3.3 | Tab and Shift+Tab move to the next and previous cell, committing first; Enter commits and closes | ✅ | Landed with Q3.2: the field's focus-traversal keys are off so Tab reaches the binding; the walk wraps to the next and previous row and closes past either end. A click on another cell commits the open one first |
| Q3.4 | `Agenstorm.EditTableAsText` in `Markdown.EditorContextMenuGroup` reveals the raw table at the caret; tests; guardrails | ✅ | The action takes the table under `PlatformDataKeys.CONTEXT_MENU_POINT`, or without a point the first rendered table at or after the caret, and moves the caret into its first cell; `update` stays on the BGT by asking only whether a rendered table exists. Shown only then. `EditTableAsTextActionTest`, four cases. Guardrails: the run below |
...
| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| One cell | Click a cell: a field over that cell with its raw Markdown, everything else still rendered; Enter writes it back and the table shows the change; Esc leaves the file untouched | ✅ | Roman, 2026-09-21, second sandbox run ("all good, continue with the 1.8.0 cut"). The first run had shown the focus-loss commit failing (the Log clean row) |
| Walk a row | Tab and Shift+Tab move across the cells, each edit committed on the way | ✅ | Same run |
| Follows | The field stays on its cell while the editor scrolls and after the editor is resized | ✅ | Same run; scrolling needs no code at all (the field is a child of the component that scrolls), a resize goes through `reposition` |
| Undo | One Undo reverts one cell edit, and the table re-renders | ✅ | Same run; `CellEditorTest` asserts the single step |
| Structural | **Edit Table as Text** opens the raw table; a row added there renders on the way out | ✅ | Same run |
| Log clean | No plugin exceptions in the session | ✅ | **First run, 2026-09-21 15:48–15:55: two SEVEREs, both ours** — "Access is allowed from write thread only" from `CellEditor.commit` reached through the field's `focusLost`: an AWT focus event is dispatched without the write-intent lock, and `PsiDocumentManager.commitDocument` asserts it. Every Swing-originated callback of the field (Enter, Esc, Tab, focus loss) now hops through `invokeLater`, which runs under the lock, and checks the cell is still the open one; `CellEditorTest` pins the deferral. `WriteIntentReadAction` would have done it in place but is `@Experimental`. **Second run, 15:58–16:00: nothing of ours.** The session's one SEVERE is the PHP plugin's `FrameworkProjectDataService` doing VFS work on the EDT at construction — zero frames of ours, blamed on the platform |

**1.8.0 (2026-09-21), cut the same day the epic was specified.** Epic Q ships **on by default** as the `Render tables in
place` switch. `patchChangelog` moved `[Unreleased]` into a `[1.8.0]` section; `build/distributions/agenstorm-1.8.0.zip`
is 907 KB, sha256 `7a67eefd…`. `verifyPlugin`: **Compatible** on PS-262.10968.76 and IU-262.10968.63, **zero internal
usages**, 76 experimental and 11 deprecated — the 1.7.0 counts. Minor rather than patch because it adds a feature. 47 new
tests across the epic; two sandbox runs, each of which found one real bug the tests could not (a Disposer leak, a commit
outside the write-intent lock), both fixed the same evening.
...
| R7 | **Rejected by review → no internal API.** Replace or remove every `@ApiStatus.Internal` usage the Marketplace review flagged, keep every feature that survives working, and re-submit | 🔄 | Code done 2026-09-10 (`b7db79d` Epic B cut · `6818098` title refresh via `UISettings` · `a536146` branch widget on `GitBranchWidget` · `f331c4b` toolbar slot via `replaceAction`). `verifyPlugin`: **Compatible on PS-262.10315.130 and IU-262.10315.125, zero internal usages** — the verifier no longer writes an `internal-api-usages.txt` at all — 46 experimental (up 4: Kotlin materialises `DynamicPluginListener`'s default methods, which are `@ApiStatus.Experimental`) and 2 deprecated. Decisions 35–38. **Guardrail run 2026-09-10:** `runIde` with a scratchpad `demo-alpha` (1 ahead / 1 behind, so the branch icon has both arrows) and `demo-beta` (on `feature/checkout`); the sandbox ran 5 h 19 m across those and two of Roman's real projects and exited clean. **Today's slice of `idea.log` has zero SEVERE/ERROR lines and no `Plugin to blame` entries** — the ones in that file are all from 2026-09-05/06, it is not rotated. Roman: "Seems all good." **Signed off on what was on screen for five hours** — the tab strip, the branch in the left corner, the window title. **Not exercised:** the feature toggles. `config/options/agenstorm.xml` was not written during the run (mtime still 2026-09-09 11:55), so nobody watched the stock project widget come back after switching project tabs off, which is the one behaviour decision 36 changed and the one tests cannot reach — `ProjectTabsWidgetActionTest` proves the `replaceAction` round trip, not the toolbar re-reading its group. Residual risk, carried knowingly: worst case the slot needs a restart to change, which is a settings-page note, not a broken feature. **Then, same day:** Roman asked for the scratch popup filtering back on public API, and Phase B2 delivered it — so nothing is removed after all and the release is a feature release. `verifyPlugin` re-run after B2: **Compatible on both IDEs, still zero internal usages**, 50 experimental and 4 deprecated. The deprecated four are Kotlin materialising `DynamicPluginListener.checkUnloadPlugin`, a deprecated default method the plugin never calls; the two the plugin used to carry (`StatusBarWidget.getPresentation` bridges) went away with the branch widget rework. **Version cut 2026-09-10 as 1.2.0** (`78e4110`…): ZIP builds, verifier green, change notes in place. **Open:** a guardrail run for the new scratch popup, and the re-submission itself |

**1.1.0 (2026-09-09).** Cut off the same branch, before 1.0.0 ever reached the Marketplace: `pluginVersion = 1.1.0`, `patchChangelog` moved `[Unreleased]` into a `[1.1.0]` section (the change notes in the built `plugin.xml` come from it), and `build/distributions/agenstorm-1.1.0.zip` builds. `verifyPlugin` 2026-09-09: **Compatible** against PS-262.10315.130 and IU-262.10315.125, 15 internal usages and 42 experimental. Every internal one is a §2 hook — `ProjectToolbarWidgetAction` (9), `GitBranchesTreePopupOnBackend` (2), `ScratchFileTypeFilter` (2), `FrameTitleBuilder` (2) — and **none** comes from `markdown/`, so Phase F4 and Phase H3 added no internal usage at all. The one new experimental usage is `MarkdownElementTypes.ALERT`, left as a plain reference like every other experimental one in the codebase; `until-build=262.*` pins the platform it exists in. Minor rather than patch because H3 adds features. What R6 publishes is now this, not 1.0.0.

**1.2.0 (2026-09-10).** The release the Marketplace review sent back, rebuilt. `pluginVersion = 1.2.0`, `patchChangelog` moved `[Unreleased]` into a `[1.2.0]` section, and `build/distributions/agenstorm-1.2.0.zip` builds at 551 KB, sha256 `932b6d86…`. **Minor, not patch:** the window title, branch widget and toolbar slot changes are invisible to users, but the scratch popup is rebuilt with language-level filtering and an "All Languages…" entry, the branch icon gains incoming/outgoing arrows, and HTML joins the default allow-list. Nothing is removed — Epic B was cut and brought back inside the same day. `verifyPlugin` 2026-09-10: **Compatible** against PS-262.10315.130 and IU-262.10315.125, **zero internal usages** (the verifier writes no `internal-api-usages.txt` at all), 50 experimental, 4 deprecated — the deprecated four are Kotlin materialising `DynamicPluginListener.checkUnloadPlugin`, which the plugin never calls. The built `plugin.xml` carries `since-build=262`, `until-build=262.*` and the 1.2.0 change notes. What R6 publishes is now this, not 1.1.0, and the 1.1.0 draft GitHub release on `12f3932` (id 385481892) is stale: its notes describe a build that was never accepted.
...
|------|------------|--------|------------|
| Public-but-impl API drift (`FrameTitleBuilder` override, `commentsReferenceProvider`, git4idea's `GitBranchWidget`, the `main.toolbar.Project` action id) breaks a feature in 2026.3+ | Med | Med | Each feature isolated behind a toggle and an optional config file; `verifyPlugin` against EAP in CI; the swap and the placement both fail soft (log + no-op) when the hook is missing. Internal API is no longer part of this risk — there is none | 
...
| 32 | 2026-09-09 | The macOS window tabs stay **on**; Agenstorm hides only their row instead of disabling them. Registry writes are limited to giving the key back to installs 1.0 turned it off in | Roman: "every tab actually opens a new window, and that should not be the case". `ide.mac.os.wintabs.version2` is read by `JdkEx.getTabbingModeInvocator()`, so under the New UI turning it off stops `MacWinTabsHandlerV2.initFrame` from calling `JdkEx.setTabbingMode` — no `NSWindowTabGroup`, one window per project. Epic E's original rationale ("re-parenting `WindowTabsComponent` breaks the bookkeeping") was sound but answered the wrong question: the row can simply be made invisible, which `CustomHeaderRootLayout` honours by giving it zero height, leaving the header (traffic lights + toolbar, laid out above it) alone and re-parenting nothing. The single-header-row goal and the single-window goal are both met. Windows and Linux have no merging to build on, so the strip stays a window switcher there (Roman, 2026-09-09) | Roman (decision), Claude (investigation, text) |
| 33 | 2026-09-09 | Live markup neutralises `FOLDED_TEXT_ATTRIBUTES` for the editors it owns, accepting that genuinely folded headings and fences there lose the scheme's background and border | The bullet • and the two checkbox glyphs are fold placeholders, and the platform paints every collapsed placeholder with that one per-editor value — `getFoldRegionAttributes` has no per-region hook. The alternatives were to keep the effect and lose only the border (leaves each bullet on a lighter block) or to move the three glyphs to inlays with their own renderer (correct, but reworks Epic F's core and moves checkbox hit-testing from `getFoldingPlaceholderAt` to `inlayModel.getElementAt`, in a shipped feature). Roman chose the scheme override for 1.0.1; inlays stay available if folded `...` ever reads badly | Roman (decision), Claude (investigation, text) |
| 34 | 2026-09-09 | A block quote gets one card and one accent bar, however deep the nesting; each `>` folds to a single space rather than being removed | Roman asked for "a similar simple approach as for code block" and chose card + bar + indent over a bare card. Folding each `>` to a space instead of to nothing keeps every column exactly where it was, so revealing the markers at the caret does not shift the line, and it makes nesting depth read as indent without a rail per level — which also keeps the renderer to one highlighter per block, as H1 has it. The alternative, one rail per level, needs a rail geometry per nesting depth and buys little once the indent already shows it | Roman (decision), Claude (text) |
| 35 | 2026-09-10 | Epic B is cut rather than rebuilt on public API | The JetBrains Marketplace review rejected 1.1.0 for internal API usage. `scratchLanguageFilter` is the only hook that filters the New Scratch File popup, and the alternative decision 14 already weighed — replacing the platform's `NewScratchFile` action with a hand-built popup — costs the LRU order, the buffers entry and selection-based language detection for a feature whose value is trimming a list. Roman: "Drop #3" | Roman |
| 36 | 2026-09-10 | The project tabs take `main.toolbar.Project` at runtime through `ActionManager.replaceAction`, not through `<action overrides="true">`, and keep the displaced widget to put back | `overrides="true"` replaces the stock action for good, which is the only reason the widget extended the internal `ProjectToolbarWidgetAction`: it needed something to delegate to while the feature was off. Swapping at runtime keeps the stock widget as a live `AnAction` the plugin never names, so feature-off is genuinely stock. It costs two listeners — `AppLifecycleListener.appFrameCreated` to take the slot before the first toolbar is built, `DynamicPluginListener.beforePluginUnload` to give it back, which the XML mechanism used to handle | Roman |
...
| 53 | 2026-09-14 | **Epic O's inline rename removes the Rename dialog, not the refactoring's own follow-up dialogs**, and the inline field is placed directly below the row that was clicked | Asked whether a rename with no dialog at all was wanted, Roman took platform-faithful. The seam is `PsiElementRenameHandler.rename(element, project, context, editor, newName)` (public, unannotated): with a non-null name it builds the rename dialog and disposes it without ever showing it, so the main dialog is gone while the element's own `RenamePsiElementProcessor` still runs — which is what lets PhpStorm substitute a file rename into a class rename. What it does **not** suppress is the automatic renamers PhpStorm registers (class inheritors, field accessors, parameters); when one of those has something to offer, its dialog still appears. Suppressing that needs a `RenameProcessor` subclass auto-accepting every renamer, and the cost was judged the wrong way round: the IDE would then rename things the user was never shown. **The placement half** is Roman's answer to which row the overlay is allowed to cover: right below the clicked row, and the target folder's last child when there was no click **Its rename half is superseded by decision 62 (2026-09-16): the automatic-renaming question is now answered as OK would, without the dialog** | Roman |
| 54 | 2026-09-14 | **A file made from the inline row is empty**, like the one the stock action makes — the file template is not applied | Built first with the template applied, on a premise I had given Roman and got wrong: that the platform's New \| File already does this. It does not. `CreateFileAction.create` is `MkDirs` followed by `directory.createFile(name)` and nothing else, so typing `Foo.php` into the IDE's own dialog gives an empty file; the `<?php` comes from PhpStorm's separate *PHP File* entry, which asks for a class name rather than a file name and is not one of the actions this epic takes. Told that, Roman dropped it: the inline row is meant to change **where you type the name**, not what you get, and a file appearing with content nobody asked for is a surprise the dialog never springs. `InlineFileTemplates` was deleted. **Lesson kept: check what the action being replaced actually does before describing the replacement as parity** | Roman |
| 55 | 2026-09-14 | **Epic O's rename is taken with `ActionSlot` on `RenameElement`, not registered on the `renameHandler` extension point.** Supersedes the rename half of decision 52 | The EP was chosen because a handler that declines gives the platform's dialog back with no code to write. Reading `RenameHandlerRegistry` before building it showed the opposite: `getRenameHandler` collects every handler whose `isRenaming` is true into a `TreeMap` keyed by `getHandlerTitle`, which for anything that is not a `TitledHandler` is `toString()` — unique per instance — so two available handlers never collapse and the user gets a **chooser dialog**. In PhpStorm a directory is already claimed by `PlainDirectoryRenameHandler` and a `.md` file by `MarkdownFileRenameHandler`, so an Agenstorm handler would have added a second dialog to the one this epic exists to remove. With a slot the fallback is a `delegate.actionPerformed(e)` call, which is one line and cannot summon a chooser. The gate on what may be renamed inline came out of the same read: both of those handlers are pure pass-throughs to `PsiElementRenameHandler.rename`, while `PhpDirectoryPackageRenameHandler` and the project/module handlers are not, so a namespace folder, a content root and the project folder keep their own dialogs. **Lesson kept: an extension point that picks between implementations has a policy for ties, and it is worth reading before assuming yours wins** | Roman |
| 56 | 2026-09-14 | **The inline rename assembles its own `RenameProcessor` and turns the usage preview off**, keeping the preview only for the trigger the platform reserves for it | Roman, from the first sandbox run: "when I rename it triggers rename refactoring shown in a toolwindow, not popup - i thought it would be hidden by default". Cause: `RefactoringDialog` initialises `myCbPreviewResults` to `true` and only the dialog's own checkbox ever clears it, so `PsiElementRenameHandler.rename` — which builds the dialog and disposes it without showing it — handed the processor `setPreviewUsages(true)` every time. Decision 53's "platform-faithful" reading had assumed the flag defaulted off. Asked whether to suppress the preview outright, Roman chose **only when it is just code usages**, so the flag is set false and `RenameProcessor.isPreviewUsages`'s other trigger, `reportNonRegularUsages`, is left alone: a rename reaching into comments or plain text still shows its list first. The processor is now built the way `RenameDialog` builds it — `RenamePsiElementProcessor.forElement` for substitution and the two search flags, plus every enabled `AutomaticRenamerFactory` — so nothing else about the refactoring changes. **Lesson kept: a dialog that is never shown still carries its defaults into whatever it constructs** | Roman |
| 57 | 2026-09-14 | **Phase O2 adds its own Duplicate action instead of taking the `CopyElement` slot, and leaves `⌘C`/`⌘V` alone** | `CopyElement` (F5) offers a *target directory* as well as a name, and a one-line field cannot: taking the slot would have removed the folder browser in order to add an inline name. Offered the choice between taking F5 with paths typed into the row, taking it for same-folder copies only, and adding a separate action, Roman took the separate action — nothing the IDE does today is removed, and the cost is that there is no shortcut until someone binds one. **No default shortcut is claimed:** the obvious one, ⌘D, is `EditorDuplicate` everywhere, and Epic J already paid once for taking a keystroke that was spoken for (decisions 45 and 46). **Paste-into-the-same-folder is out of scope:** `$Paste` routes through the project view's own `PASTE_PROVIDER`, and there is no seam that reaches the tree without replacing `$Paste` globally, which would put Agenstorm in the path of every paste in the IDE. Unlike rename, the `copyHandler` EP would have been safe — `CopyHandler.doCopy` takes the first delegate whose `canCopy` is true and never shows a chooser — but it is not needed for an action that takes nothing over | Roman |
| 58 | 2026-09-15 | **The tree opens a gap for the row being named, by doubling the anchor row's height through a wrapping cell renderer** | Roman, on the first full run: "when I try adding a file or a directory, it creates a node for typing, but it is placed over the existing nodes. Instead it should move apart the other nodes and add this new one inside" — with a screenshot of the field sitting on top of `test.md`. Decision 52 had accepted the covered row as the price of not being able to insert into a structure-driven model, and that half was wrong: a row cannot be *added*, but a row can be made *taller*, and `JTree.setCellRenderer` makes `BasicTreeUI` recompute every row's size. `SpacerRenderer` wraps the tree's own renderer and returns a panel one row taller for the anchor alone, so everything below moves down by exactly one row and the field sits in the empty half. Plain Swing on a component the tree already exposes — `getCellRenderer` out, `setCellRenderer` back on close — and every other row still goes through the project view's own renderer, so colours, icons and selection are untouched. **Lesson kept: "the model will not let me" is not the same as "the layout will not let me"** **Superseded by decision 61 the next day: it never worked. `DefaultTreeUI.getNodeDimensions` sizes a row from the renderer only when `getRowHeight()` is 0, the project tree sets a fixed height, and the spacer's taller row was ignored — shipped having tested only the arithmetic** | Roman |
| 59 | 2026-09-15 | **The New menu's typed entries are taken too, found by name**, and the plain *File* entry still makes an empty file | Roman, second run: "New typed file (like PHP, or HTML, or whatever) still works with a popup - fix", reversing the *File and Directory only* scope he set before seeing it. There is no clean way to do this: every such entry extends `CreateFileFromTemplateAction`, whose `buildDialog` is `protected`, so an entry cannot be asked which template it uses, and their dialogs are not all alike — HTML's offers HTML 5, HTML 4 and XHTML. What is reliable is the convention that an entry named *PHP File* is backed by a file template named *PHP File*, so the scan takes entries whose name ends in ` File` and each action resolves its template at invocation, falling back to its own dialog when nothing answers. That filter turns out to do the hard work: of the seven entries found, `NewComposerJsonFile` and `CreateEditorConfigFile` resolve nothing and are left exactly as they were. **The cost, taken openly:** *HTML File* now makes whatever the *HTML File* template is, so its HTML 4 / XHTML choice is gone. *PHP Class*, *Interface* and *Trait* are untouched — they are not named `… File`, and their namespace field is the reason the line is drawn there. **Decision 54 stands:** the plain *File* entry still makes an empty file, because the stock one does | Roman |
...
| 70 | 2026-09-22 | **Light fold regions live in the reworked terminal's output editor, so Epic I renders with them (I2.1 goes ahead; I2.3 stays the floor for the classic engine).** Driven through the robot server in a `runIdeForUiTests` sandbox — a reworked tab opened with the frontend `TerminalToolWindowTabsManager`, commands sent with `TerminalView.sendText`, fold regions read back per step — the spike controller (built-in rules, one collapsed light region per block) held up in every case: **appended below** — the region stays, new ones join it; **the last line rewritten with CR** (a progress bar) — untouched; **resize** — `setMaximized` on and off rewrote the tail of the document, the regions kept their offsets; **the alternate buffer** — `less` renders into a *second* editor (`isAlternateBufferModelEditor`), the output editor and its regions are untouched, and `q` brings the prompt back under them; **the terminal's own decoration pass** — no conflict seen over 175 k lines; **commandFinished** — every prompt return left the regions in place; **the placeholder is clickable** — a synthetic click on it expanded the region, renderer mode and viewer flag notwithstanding; **scrollback trimming** — the reworked buffer trims by *characters*, at 1 MiB (≈175 k lines of `seq`), not at a line count, and it cuts mid-line: the six regions above the cut were gone afterwards and the one below kept its text, so the controller's rule (a pure deletion at offset 0 shifts the scan position; a region left starting at 0 is dropped) is the right one — the limit is the advanced setting *New terminal output capacity*, 1024 KB by default, so whatever a user sets there is what the trim handling meets. **Not exercised:** Clear (⌘K) — `Terminal.ClearBuffer` is disabled outside the terminal's own focus context, so it is Roman's eye check; and the terminal was not *typed into* by a person. Two things learned on the way: the shell of a tab does not start until the tool window is showing, and the frontend classes are only reachable from another plugin's classloader through an instance (the `Terminal.NewTab` action's class gave the robot script its loader) | Claude (the run and the text), Roman (to confirm with ⌘K and by eye) |
| 71 | 2026-09-22 | **Epic I targets the Reworked 2025 terminal engine only.** I2.3 (the `consoleFilterProvider` floor for the classic engine) is cut, and the *Both engines* guardrail becomes *Classic engine: nothing attaches, nothing throws*. PhpStorm 2026.2 ships three engines — Classic (JediTerm), Reworked 2025 and the deprecated Experimental 2024 — and no Ghostty one (the only "ghostty" string in the IDE is the debugger recognising the Ghostty app as an external terminal). Roman uses Reworked 2025; the filter could only highlight a block's opener there, never fold, and the classic engine is on its way out. "For now": a classic-engine floor can come back as its own step if anyone asks | Roman ("for now reworked only"), Claude (the survey and the text) |

---

## 7. Open questions

- [ ] **Epic P, `TerminalCommandGuard` on the classic engine (seen 2026-09-22 in the I1.3 sandbox):** with a classic JediTerm tab open, the offload sweep's `isRunning` trips two `ThreadingAssertions` SEVEREs blamed on Agenstorm — `TerminalWidget.isCommandRunning()` asserts a background thread and `ShellTerminalWidget.hasRunningCommands()` asserts no read access — because `Offloader.sweep` runs on the EDT under the write-intent lock. 24 in one session, none ever in Roman's daily-driver log (reworked engine only). Fix: ask the guards off the EDT ahead of the sweep, or drop the classic branch. Not urgent; a classic-engine user would see it.
- [x] ~~**Epic O, Phase O2 (duplicate / paste-a-copy):** through the `com.intellij.refactoring.copyHandler` EP or through an `ActionSlot` on `CopyElement`?~~ **Resolved 2026-09-14 — neither** (decision 57). O2 adds its own `Agenstorm.DuplicateInline` action, so `CopyElement` keeps its dialog and its target-directory browser. The EP was checked and would have been safe (`CopyHandler.doCopy` is first-wins, no chooser, unlike `RenameHandlerRegistry`), but an action that takes nothing over does not need it. Paste-into-the-same-folder is out: `$Paste` has no seam that reaches only the tree.
...
- [x] **Epic D, clearing the message box after a commit — settled 2026-09-10, no plugin code.** Roman: "after commit has been committed, cleanup commit message text box". The platform already does it. `AbstractCommitMessagePolicy.onAfterCommit()` sets the field to `CommitMessage.EMPTY` and calls `cleanupStoredMessage()` when `VcsConfiguration.CLEAR_INITIAL_COMMIT_MESSAGE` is on; otherwise it refills the field from the changelist description, which `ChangeListCommitMessagePolicy` has been writing the typed message into all along — that is why the just-committed message reappears. The flag is Settings | Version Control | Commit → "Clear initial commit message", off by default, and in build 262 `AbstractCommitMessagePolicy` is its only reader with behaviour attached (the other three hits are `CommitDialogConfigurable`, `VcsOptionsTopHitProvider` and `VcsOptionsUsagesCollector`), so turning it on does this and nothing else. Roman ticks the checkbox; Agenstorm neither surfaces nor forces the flag.
