...
**Now on:** **Epic K — the IDE is the terminal's editor** (`$EDITOR`, Ctrl+G), requested as IJPL-221866 and scoped with Roman on 2026-09-10 (decision 43: on by default, forced past the rc file, tab close ends the wait). **K1.1 through K1.4 are done** — the shim is generated, `EDITOR`/`VISUAL` point at it forced past the rc files, `POST /edit` holds its answer, and the file opens in the terminal's own project and is handed back when its tab closes. The round trip works end to end in tests, including the flush barrier of decision 44. What is left is **K1.5** (a settings group of its own and the one-time balloon) and **K1.6** (README, changelog, and the correction to the YouTrack comment), then the epic's guardrails — which need Roman in a sandbox with a real Claude Code session. The release track below is unchanged and still Roman's.
