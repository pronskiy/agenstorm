...
**Now on:** **Epic Q — Markdown tables rendered in place**, added 2026-09-21 at Roman's request ("I want to be able to view markdown tables really nicely" — the raw table "looks like complete mess"). Spec'd in one brainstorm; the four calls are decisions 66–68. **Phase Q1 is closed** (2026-09-21: `TableModel` + `TableModelBuilder` with thirteen tests, `TableLayout` with nine, both guardrails ✅). **Q2.1–Q2.4 landed 2026-09-21.** **Next actionable step: Q2.5** — the `markdown.tables` settings row, bundle keys, README and CHANGELOG; then the Phase Q2 guardrail run in `runIde` with Roman. Q1 is tests-only; the first `runIde` sign-off is the Phase Q2 guardrail table.
