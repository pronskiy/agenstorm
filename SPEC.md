...
**Now on:** **Epic K — the IDE is the terminal's editor** (`$EDITOR`, Ctrl+G), requested as IJPL-221866 and scoped with Roman on 2026-09-10 (decision 43: on by default, forced past the rc file, tab close ends the wait). **K1.1 and K1.2 are done** — the `agenstorm-edit` shim is generated, and a new terminal now carries `EDITOR`/`VISUAL` pointing at it, forced past the rc files, with the editor they displaced kept as the shim's fallback. Nothing answers `/edit` yet, so a Ctrl+G still lands in that fallback: **K1.3**, the endpoint, is next, and **K1.4** is what makes the round trip real. The release track below is unchanged and still Roman's.
