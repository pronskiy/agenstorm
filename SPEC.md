...
**Now on:** **the guardrail lines nobody has exercised, then a version cut.** Epics M and N are both built, committed, `check`-green and `verifyPlugin`-clean (zero internal on both IDEs), and Roman has signed off what each one does: "works now" for M, "All good, leave as is" for N. What is left is five unexercised lines across the two — M's *Markdown only*, *No restart* and *Reachable*, N's *Both ways, no restart* and *Not greedy* — plus a confirming run for N's *No fight* fix. **One of them may be a real problem:** M's wrapper hides the layout actions from Find Action too while a Markdown file is current, which is the escape hatch the README promises. Settle that before cutting a version; `pluginVersion` is still 1.4.0.
