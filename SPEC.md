...
| O1.3 | `InlineCreateAction` + `ProjectViewActionInstaller` + the two lifecycle listeners | 🔄 | One action class serving both slots, told apart by the [InlineNameKind] it is built with. Commit is one `WriteCommandAction` with one group id, so the folders and the file a `src/Http/Client.php` made undo together. **A file gets its file template, which the stock action does not do** — `CreateFileAction.create` is `directory.createFile(name)` and nothing else, so the platform's New \| File gives an empty `.php`; the `<?php` comes from PhpStorm's separate *PHP File* entry. Tests still owed |
