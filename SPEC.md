...
| Module layout | One Gradle module, one plugin, feature packages `links`, `frame`, `commit`, `tabs`, `markdown`, `terminal`; optional dependencies wired through `<depends optional="true" config-file="…">` | Keeps a single artifact while letting the plugin load in IDEA/WebStorm without PHP/Markdown/Terminal |
...
### Epic B — New Scratch File allowlist  ·  ❌ Cut 2026-09-10

**Shipped in 1.0.0 and 1.1.0, removed before the next release.** It was the only feature whose hook was
`@ApiStatus.Internal` with no public replacement: `scratchLanguageFilter` is the sole way to filter that popup
short of replacing the platform's `NewScratchFile` action with a hand-built one (no LRU order, no
selection-based language detection — the trade decision 14 already refused). The Marketplace review rejected
the plugin over internal API usage, so the feature went rather than the plugin's chance of being listed.
Decision 35. Everything below is the record of what was built and why it worked; the code is gone as of
`b7db79d`, along with `scratchFilterEnabled`, `scratchAllowedFileTypes` and the "Scratch files" settings group.

**Goal (was):** The New Scratch File popup shows only Plain text, Markdown, PHP and JavaScript (configurable).
**Success metrics (were):** Popup shows exactly the allowlisted types; feature off → platform default list.
