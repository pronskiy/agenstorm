...
**Now on:** **1.5.0 is cut and waiting to be published.** `pluginVersion = 1.5.0`, `patchChangelog` moved `[Unreleased]` into a `[1.5.0]` section, `build/distributions/agenstorm-1.5.0.zip` builds at 660 KB (sha256 `516d9b14…`), and `verifyPlugin` is **Compatible on PS-262.10315.130 and IU-262.10315.125 with zero internal API** (58 experimental, 8 deprecated). Publishing is Roman's: push, then publish the draft release, which runs `release.yml`. **Two things carried knowingly into this release:** five guardrail lines across M and N were never exercised (M's *Markdown only*, *No restart*, *Reachable*; N's *Both ways*, *Not greedy*), all covered by unit tests; and M hides the layout actions from Find Action too, so while it is on there is no way to open the preview — documented in the README and the settings comment rather than fixed.
