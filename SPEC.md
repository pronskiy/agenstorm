...
| Fresh install | R4 tour clean; the plugin loads without the PHP plugin (no PHP-only features shown) and with the Terminal plugin disabled | 🔄 | "R3 tour" was a stale reference to the pre-renumbering steps — the hand-install tour is **R4**, and it passed 2026-09-07 after the `17d128b` threading fix, log clean. What is left is the optional-dependency half. `verifyPlugin` is Compatible against **IU-262.10315.125**, an IDE with no PHP plugin, which proves the plugin *resolves* without it but not that no PHP-only feature is offered at runtime. Only PhpStorm 2026.2 EAP is installed on this machine, so rather than install IntelliJ IDEA the cheaper check is inside PhpStorm: disable the PHP plugin, then the Terminal plugin, restart each time and confirm the plugin still loads, the PHP-only settings stay hidden and `idea.log` is clean. Roman's, by hand |
