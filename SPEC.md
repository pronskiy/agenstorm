...
**Now on:** **Release 1.0** → step **R2** (plugin icon in `META-INF`, and the Marketplace screenshots).

Epics 0–F closed 2026-09-06, Phase F3 included. Roman added three more features to 1.0 that day; **Epic G** closed and **Epic H** stopped after Phase H1, both on 2026-09-06. Roman then cut 1.0 short: **the release goes next**, and everything still open — Epic H's Phases H2 and H3, and the whole of Epic I — moves to **after 1.0**. So the order is **G ✅ → H1 ✅ → Release 1.0 → H2/H3 + I**. Release 1.0 runs R1–R6 in order. **R1 closed 2026-09-07**: version, changelog, README description and the CLAUDE.md inventory now match the shipped build. Next is R2 (icon and Marketplace media), which has to land before R3 because the icon ships inside the ZIP; then R3 (a `verifyPlugin` re-run for Epic G's Terminal dependency) and R5 (the GitHub repo, where `origin` and `pluginRepositoryUrl` disagree). R4 (hand-install tour) and R6 (Marketplace) are Roman's. Decisions 25–28 were confirmed by Roman on 2026-09-06 and 29–31 came out of the Epic G and H guardrail runs.
...
| R2 | Plugin icon and Marketplace media: `src/main/resources/META-INF/pluginIcon.svg` (40×40, plus `pluginIcon_dark.svg`), and the listing images — screenshots of the features 1.0 actually ships | 🔲 | **Nothing exists yet**: there is no `pluginIcon*.svg` in `META-INF/`, so the plugin currently shows the platform's default placeholder in the Plugins list and on the Marketplace card. The icon ships inside the ZIP and so must land before R3; the screenshots are uploaded to the Marketplace listing separately and are only needed for R6. Screenshots worth having, one per shipped feature: a clickable `path:line:col` in a Markdown file, the commit-message action mid-stream, the project tabs in the toolbar, live markup with a code-block card, and `open src/Foo.php:42` in a terminal |
| R3 | `./gradlew verifyPlugin` against the recommended PhpStorm and IntelliJ IDEA 2026.2 builds: Compatible on both; internal-API usages limited to the §2 list | 🔄 | Done 2026-09-06 for the Epics 0–F build; must be re-run now that G and H1 have landed (new optional dependency on the Terminal plugin, the `ShellExecOptionsCustomizer` and `LocalEelDescriptor` `@Experimental` usages, no new internal ones expected). The 2026-09-06 result, plugin 1.0.0: Compatible on PS-262.10315.130 and IU-262.10315.125. 15 internal usages, all from the §2 list (`ProjectToolbarWidgetAction` subclassing and overrides, `IdeFrameEx.setFileTitle`, `ScratchFileTypeFilter`, `GitBranchesTreePopupOnBackend.create`); 28 experimental usages (Symbol API, `PsiHighlightedReference`, the Markdown plugin's `MarkdownInlineLink` / `MarkdownLinkText`); 2 deprecated usages, both the Kotlin bridge for `StatusBarWidget.getPresentation(PlatformType)`. A third deprecated usage, `DaemonCodeAnalyzer.restart(PsiFile)`, was replaced by the overload with a reason. Reports in `build/reports/pluginVerifier/`; the ZIP in `build/distributions/agenstorm-1.0.0.zip` |
| R4 | `./gradlew buildPlugin`, install `build/distributions/agenstorm-1.0.0.zip` into a real PhpStorm 2026.2 from disk, open a project with Markdown, Git and PHP: every feature toggle visible in Settings, no SEVERE in `idea.log` after a short tour | 🔲 | Roman's, by hand |
| R5 | GitHub repo: the code pushed to the repository the metadata names, description and topics set, the Build workflow green there, the release secrets in place, and `v1.0.0` tagged so the Release workflow can pick the draft up | 🔲 | **Two things are wrong today.** `origin` is `git@github.com:pronskiy/agent-control.git`, while `pluginRepositoryUrl`, the README badge and the README install link all say `pronskiy/agenstorm` — the changelog plugin builds its compare links from that property, so one of the two has to give. And `main` is **201 commits ahead of `origin/main`**: nothing has been pushed, so no workflow has ever run on GitHub and there are no tags. Also needs the four secrets `release.yml` expects (`PUBLISH_TOKEN`, `CERTIFICATE_CHAIN`, `PRIVATE_KEY`, `PRIVATE_KEY_PASSWORD`) and a decision on whether `run-ui-tests.yml` stays, since nothing in the project writes UI tests |
| R6 | Marketplace: upload the ZIP (or let the Release workflow run `publishPlugin` off the GitHub release), release notes from the changelog, listing text and the R2 screenshots | 🔲 | Roman's. The `v1.0.0` tag and the draft release belong to R5; this step is what turns that draft into a published release and a Marketplace listing |
