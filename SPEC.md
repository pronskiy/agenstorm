...
| Reachable | Find Action → "Preview Only" still switches the layout while the buttons are hidden | 🔲 | Not exercised. Note the wrapper hides the action from **everywhere**, Find Action included, while a Markdown file is current — so the escape hatch the README promises may not hold. Worth checking before this is released |
