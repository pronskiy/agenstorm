...
| Step | Description | Status | Notes |
|------|-------------|--------|-------|
| I2.1 | `TerminalEnhancerController`: light fold regions collapsed to the summary, per output editor | 🔄 | Unblocked by decision 70; the spike controller is the first cut. Left: the gutter icon, the `FoldingListener` re-apply, the settings toggle wired through `AgenstormSettingsListener`, the viewer hook (I2.2), tests for the trim arithmetic |
