...
**Now on:** **Epic O — inline filename typing in the project tree. Both phases are built; the guardrails are what is left.** O1.0–O1.7 and O2.1–O2.4 are ✅: `check` green, `verifyPlugin` **Compatible on both IDEs with zero internal API**, and the deprecated and experimental counts unmoved by Phase O2. Rename is signed off — **Roman, 2026-09-14: "rename works now"** — after decision 56 took the usage preview off, which is the defect the first sandbox run found. Everything else in the guardrail table still needs a person: the row's height, indent and icon, Escape and click-away, the nested path, Duplicate on a file and on a folder, and the off switch. Six decisions, 52–57, four of them reversing something planned.
