...
| O1.3 | `InlineCreateAction` + `ProjectViewActionInstaller` + the two lifecycle listeners | 🔄 | One action class serving both slots, told apart by the `InlineNameKind` it is built with. Commit is one `WriteCommandAction` with one group id, so the folders and the file a `src/Http/Client.php` made undo together. **The new file is empty**, which is what the stock action does — decision 54. Tests still owed |
| O1.4 | `InlineRenameAction` in the `RenameElement` slot, and `InlineRename`'s gate | 🔄 | Not the `renameHandler` EP — decision 55. The gate: feature on, the event came from the project tree, one selected `PsiFile`, or a `PsiDirectory` that is plain, not a content root and not the project's own folder. Everything else goes to the displaced action, so Shift+F6 in the editor and on a PSR namespace folder is untouched. Tests still owed |
