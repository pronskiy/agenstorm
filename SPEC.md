...
| 2026-09-10 | Epic J's binding moved to ⌥⇧F12 on every platform after Roman found ⌘⌥M firing Extract Method; decision 45 supersedes 40's keystroke, and the check is now a test that asks the keymaps | Claude (investigation, text), Roman (report) |
...
| J1.4 | Tests | ✅ | 9 cases: registration and text, the binding is **non-empty** *and* claimed by nothing else in `$default`, both macOS keymaps and macOS System Shortcuts (decision 45), the tool-window id pinned to `TerminalToolWindowFactory.TOOL_WINDOW_ID`, `update()` hidden with the feature off and without a project, and every row of the state table |
