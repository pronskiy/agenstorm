...
**Now on:** **Epic K — the IDE is the terminal's editor** (`$EDITOR`, Ctrl+G), requested as IJPL-221866 and scoped with Roman on 2026-09-10 (decision 43: on by default, forced past the rc file, tab close ends the wait). **Phase K1 is built and its core is signed off**: Roman's 2026-09-10 runs confirmed the round trip on a plan ("so it works") and, after K1.7 was rewritten to measure rather than trust `isMaximized`, the terminal stepping aside and coming back ("cool it works"). Decision 44's flush barrier — the answer waits for the saved bytes to reach the file system — is the one to remember from this epic; it was a real data-loss bug the tests caught. **What is left is guardrails only, all Roman's**: two projects open (does the file land in the right window), a profile that exports its own `EDITOR`, `git commit` with no `-m`, the non-project-file bar, Ctrl+C mid-edit, the off switch, and a daily-driver run for the log. Plus posting the correction to IJPL-221866. The release track below is unchanged and still Roman's.
