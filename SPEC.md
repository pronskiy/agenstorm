...
**Now on:** **Epic K — the IDE is the terminal's editor** (`$EDITOR`, Ctrl+G), requested as IJPL-221866 and scoped with Roman on 2026-09-10 (decision 43: on by default, forced past the rc file, tab close ends the wait). **K1.1 through K1.4 are done** — the shim is generated, `EDITOR`/`VISUAL` point at it forced past the rc files, `POST /edit` holds its answer, and the file opens in the terminal's own project and is handed back when its tab closes. The round trip works end to end in tests, including the flush barrier of decision 44. **K1.5 and K1.6 are done too**: the bridge has its own settings group and its one-time balloon, and the README, the changelog and the CLAUDE.md inventory describe it. **Phase K1 is built**, and **K1.7** was added after Roman's first run and rewritten after his second: the terminal steps aside when it is covering the editor — measured, not taken from `isMaximized`, which a dragged splitter never sets — and comes back when the tab closes. What is left is the epic's exit guardrails, which need Roman in a sandbox with a real Claude Code session — plan mode, an already-open file and the maximized terminal are the three the tests cannot reach — plus posting the correction to IJPL-221866. The release track below is unchanged and still Roman's.
