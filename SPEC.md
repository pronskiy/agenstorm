...
| K1.7 | A maximized terminal steps aside for the edit, and takes its place back when the tab closes | ✅ | From Roman's first run: "when the terminal is maximized, I don't see the plan unless I manually minimize terminal". A maximized terminal *is* the editor's area (Epic J's geometry bullet), so the file opened behind it. Reuses Epic J's `terminalOf`/`stateOf`/`TerminalWindowState` rather than restating what maximized means, and follows the same rule as decision 42: only the terminal Agenstorm moved is moved back, and only if it is still docked, still visible and not maximized again. Whatever maximized it counts — this feature's toggle or the platform's Ctrl+Shift+' — since what matters is that the editor has no room. The restore is `NonCancellable`: a layout Agenstorm changed is one Agenstorm gives back |
...
| Maximized terminal | With the terminal maximized, Ctrl+G shows the file — the terminal steps aside — and closing the tab maximizes the terminal again. Hiding or floating the terminal while the file is open leaves that choice standing | 🔲 | |
