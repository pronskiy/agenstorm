...
| Log clean + verifier | No plugin exceptions; `verifyPlugin` Compatible with zero internal usages, the experimental count moved by `isCommandRunning` only | ✅ | Session 2026-09-19 22:29:16–22:30:47: zero SEVERE/ERROR, **zero write-intent or TransactionGuard lines** — `Dispatchers.EDT` satisfies `closeAndDispose` (the P2.5 open point); the only WARNs are the platform's `GitRepositoriesHolder`. Verifier 2026-09-19 22:36: **Compatible on PS-262.10968.76 and IU-262.10968.63, no internal-API section**, 10 deprecated (unchanged), experimental 65 → 76 — the plugin's own share from this epic is `HoverListener` (its constructor and `addTo`, the hover fix) and `TerminalWidget.isCommandRunning`; the rest is the newer 262.10968 builds the verifier now resolves marking more of what the plugin already used |
