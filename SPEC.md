...
**Now on:** Epic E → Phase E1 → step **E1.5** (verifier check of the `main.toolbar.Project` override), then the Phase E1 guardrails. Epic D is done except the **Daily-driver test** guardrail, which is Roman's over the coming commits (and owes a live run of the Anthropic and OpenAI-compatible backends with real keys).
