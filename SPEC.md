...
| D2.5 | Backend tests against a local `com.sun.net.httpserver.HttpServer` serving canned SSE; CLI test with a fake `claude` script | ✅ | Shipped with each step: `SseReaderTest` 5, `HttpSseClientTest` 4, `AnthropicBackendTest` 6, `OpenAiCompatibleBackendTest` 6, `ClaudeCliBackendTest` 10; fixtures in `testData/commit/` |
