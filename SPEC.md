...
**Now on:** **Epic O — inline filename typing in the project tree. Three phases built; the guardrails are what is left.** O1, O2 and O3 are ✅: `check` green, `verifyPlugin` **Compatible on both IDEs with zero internal API and zero override-only**, 62 experimental and 10 deprecated, unmoved since Phase O1. Rename is signed off. The second run found two more defects, both fixed: the field sat on top of the row below it, and the New menu's typed entries still opened dialogs — decisions 58 and 59. What is still unwatched is most of the guardrail table: the gap opening cleanly, Escape and click-away, the nested path, the typed entries, Duplicate, and the off switch. Eight decisions, 52–59, five of them reversing something planned or something I had told Roman.
