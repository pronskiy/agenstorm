...
| Guardrail | Criteria (pass/fail) | Status | Actual outcome |
|-----------|----------------------|--------|----------------|
| One window | With 3 projects open, the projects share one window: one entry in Mission Control, one set of traffic lights, clicking a tab selects it in place and never raises a second window | 🔲 | Needs a `runIde` run after the fix (decision 32). Also confirm `ProjectUtil.focusProjectWindow` selects the tab in the group; if it does not, switch to the group's `setSelectedWindow:` / `MacWinTabsHandler.switchFrameIfPossible`
