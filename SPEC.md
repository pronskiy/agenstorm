...
**Now on:** **Release 1.0** → step **R3** (`./gradlew verifyPlugin` re-run against PhpStorm and IntelliJ IDEA 2026.2, now that Epic G and Phase H1 have landed).

Epics 0–F closed 2026-09-06, Phase F3 included. Roman added three more features to 1.0 that day; **Epic G** closed and **Epic H** stopped after Phase H1, both on 2026-09-06. Roman then cut 1.0 short: **the release goes next**, and everything still open — Epic H's Phases H2 and H3, and the whole of Epic I — moves to **after 1.0**. So the order is **G ✅ → H1 ✅ → Release 1.0 → H2/H3 + I**. Release 1.0 runs R1–R6 in order. **R1 and R2 closed 2026-09-07**: version, changelog, README description and the CLAUDE.md inventory now match the shipped build, and the plugin has an icon in the ZIP instead of the platform placeholder. Next is R3 (a `verifyPlugin` re-run for Epic G's Terminal dependency), then R5 (the GitHub repo, where `origin` and `pluginRepositoryUrl` disagree). R4 (hand-install tour), R6 (Marketplace, which now also carries R2's listing screenshots) and the Marketplace listing text are Roman's. Decisions 25–28 were confirmed by Roman on 2026-09-06 and 29–31 came out of the Epic G and H guardrail runs.
...
| R6 | Marketplace: upload the ZIP (or let the Release workflow run `publishPlugin` off the GitHub release), release notes from the changelog, listing text and the listing screenshots | 🔲 | Roman's. The `v1.0.0` tag and the draft release belong to R5; this step is what turns that draft into a published release and a Marketplace listing. It also carries the screenshots R2 left open — one per shipped feature: a clickable `path:line:col` in a Markdown file, the commit-message action mid-stream, the project tabs in the toolbar, live markup with a code-block card, and `open src/Foo.php:42` in a terminal. They are uploaded to the listing, not built into the ZIP, so they were not needed before now |
