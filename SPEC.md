...
| Markdown only | A non-Markdown split editor still shows its layout buttons | 🔲 | **Not exercised.** `.editorconfig` is the easy case to check in the demo project — it is the one bundled non-Markdown split-preview editor in PhpStorm. Pinned by `testANonMarkdownFileIsNotHidden`, but nobody has watched it |
| No restart | Toggling the setting takes effect on the editors that are already open, both ways | 🔲 | Not exercised; the setting was on before the run started and was never toggled during it. `ActionSlot` fires `CustomActionsListener.fireSchemaChanged()`, so toolbars and tabs re-read their groups |
| Reachable | Find Action → "Preview Only" still switches the layout while the buttons are hidden | ⏸️ | **Skipped for now — Roman, 2026-09-16** (decision 60). Not exercised. Note the wrapper hides the action from **everywhere**, Find Action included, while a Markdown file is current — so the escape hatch the README promises may not hold. Worth checking before this is released |
