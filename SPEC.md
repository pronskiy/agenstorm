...
| O1.2 | `ProjectTreeAccess` + `InlineNameEditor` + `SpacerRenderer`: the field and the gap it sits in | ✅ | `ProjectTreeAccess` is the only file that names a project-view type, so the platform surface is one import list. The field is added as a child of the tree and positioned absolutely, which is how Swing mounts its own editing container; it scrolls with the tree because it is in it. **The tree opens a real gap for it** (decision 58): a wrapping cell renderer returns the anchor row one row taller, which moves everything below it down — the first version covered the row beneath instead, which is what Roman's screenshot caught. Geometry comes out of a pure function — rectangles in, a rectangle out — so the placement rule above is testable without a tree. Height always comes from a `Rectangle`: IntelliJ trees run variable-height and `getRowHeight()` returns 0 |
