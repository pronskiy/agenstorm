...
| O1.2 | `ProjectTreeAccess` + `InlineNameEditor`: the overlay field | ✅ | `ProjectTreeAccess` is the only file that names a project-view type, so the platform surface is one import list. The field is added as a child of the tree and positioned absolutely, which is how Swing mounts its own editing container; it scrolls with the tree because it is in it. Geometry comes out of a pure function — rectangles in, a rectangle out — so the placement rule above is testable without a tree. Height always comes from a `Rectangle`: IntelliJ trees run variable-height and `getRowHeight()` returns 0 |
