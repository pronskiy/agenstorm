...
| Background pause | Trigger a balloon, switch to another app for a minute, come back → it is still there, and *then* fades | 🔲 | **The one that genuinely needs a human.** The branch is pinned by a pair of tests — with `startNow = false` the smart delay is ours and no countdown is running, with `true` both are set — but whether real activation and AWT events behave that way is only answerable in a running IDE |
